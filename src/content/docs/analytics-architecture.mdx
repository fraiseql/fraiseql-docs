...
CREATE TABLE tf_sales (
    ...
    fk_customer BIGINT NOT NULL,        -- FK for JOINs
    fk_product  BIGINT NOT NULL,        -- FK for JOINs
...
CREATE INDEX idx_tf_sales_customer ON tf_sales(fk_customer);
CREATE INDEX idx_tf_sales_product ON tf_sales(fk_product);
...
CREATE INDEX idx_ta_sales_daily_year_month ON ta_sales_daily(year, month);
CREATE INDEX idx_ta_sales_daily_region ON ta_sales_daily(customer_region);

-- Population procedure (runs hourly/daily)
CREATE OR REPLACE PROCEDURE sync_ta_sales_daily()
LANGUAGE plpgsql
AS $$
BEGIN
    INSERT INTO ta_sales_daily (
        day, customer_region, product_category, sales_channel,
    ...
    ON CONFLICT (day, customer_region, product_category, sales_channel)
    DO UPDATE SET
        ...
        sync_count = ta_sales_daily.sync_count + 1,
        is_stale = FALSE;
