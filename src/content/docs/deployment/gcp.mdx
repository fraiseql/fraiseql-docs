...
docker tag fraiseql:latest \
  $REGION-docker.pkg.dev/$PROJECT_ID/fraiseql/server:latest
docker push $REGION-docker.pkg.dev/$PROJECT_ID/fraiseql/server:latest
```
...
<Aside type="tip">
Cloud Run integrates natively with Secret Manager. Secrets are mounted as environment variables at container startup with no code changes required.
</Aside>
...
```bash
# Run Confiture as a Cloud Run Job (one-off)
gcloud run jobs create fraiseql-migrate \
  --image python:3.13-slim \
  --set-secrets="DATABASE_URL=fraiseql-database-url:latest" \
  --command="sh,-c,pip install -q fraiseql-confiture && confiture migrate up --env production" \
  --region us-central1

# Execute the migration job
gcloud run jobs execute fraiseql-migrate --region us-central1 --wait
```
...
gcloud run deploy fraiseql \
  --image=$REGION-docker.pkg.dev/$PROJECT_ID/fraiseql/server:latest \
  --region=$REGION \
  --platform=managed \
  --memory=1Gi \
  --cpu=1 \
...
Cloud Run supports HTTP/2 natively, which means gRPC works without any additional configuration. FraiseQL serves GraphQL, REST, and gRPC from the same container on port 8080. When you deploy to Cloud Run with `--allow-unauthenticated`, all three transports are reachable at the same URL.

For gRPC clients connecting to a Cloud Run service URL (e.g., `fraiseql-abc123-uc.a.run.app`), use `grpc.Dial` with TLS — Cloud Run terminates TLS and forwards HTTP/2 frames to the container:

```bash
# Verify gRPC connectivity (requires grpcurl)
grpcurl fraiseql-abc123-uc.a.run.app:443 list
```

If you map a custom domain, the Cloud Run domain mapping also supports HTTP/2 and gRPC at the same custom domain. No separate load balancer rule is needed.
...
spec:
  template:
    ...
    spec:
      ...
      containers:
        - image: us-central1-docker.pkg.dev/PROJECT/fraiseql/server:latest
```
...
FraiseQL exports traces via OpenTelemetry. To send traces to Cloud Trace, configure the
OpenTelemetry exporter endpoint when deploying:

```bash
gcloud run services update fraiseql \
  --set-env-vars="OTEL_EXPORTER_OTLP_ENDPOINT=https://telemetry.googleapis.com" \
  --region=$REGION
```
...
substitutions:
  _IMAGE_NAME: 'us-central1-docker.pkg.dev/$PROJECT_ID/fraiseql/server:$SHORT_SHA'
  _REGION: 'us-central1'
...
<Aside type="tip">
**Recommendation**: Use Cloud Run for most applications, especially if traffic is variable. Switch to GKE for predictable high traffic, complex deployments, or multi-region requirements.
</Aside>
...
<CardGrid>
  <Card title="Cloud Build CI/CD" icon="setting">
    Set up Cloud Build triggers for automatic deployments on every push to main.
    [Deployment Overview](/deployment)
  </Card>
  <Card title="Cloud Monitoring" icon="open-book">
    Configure Cloud Monitoring dashboards and alert policies.
    [Scaling Guide](/deployment/scaling)
  </Card>
  <Card title="Cloud Armor" icon="warning">
    Set up Cloud Armor DDoS protection and security policies.
    [Deployment Overview](/deployment)
  </Card>
  <Card title="Troubleshooting" icon="document">
    Debug Cloud Run deployment failures and Cloud SQL connection issues.
