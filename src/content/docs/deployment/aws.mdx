...
Before deploying FraiseQL, apply pending schema migrations using Confiture:

```bash
# Run as a one-off ECS task (or locally with production DATABASE_URL)
docker run --rm \
  -e DATABASE_URL="$DATABASE_URL" \
  python:3.13-slim \
  sh -c "pip install -q fraiseql-confiture && confiture migrate up --env production"

# Check migration status
docker run --rm \
  -e DATABASE_URL="$DATABASE_URL" \
  python:3.13-slim \
  sh -c "pip install -q fraiseql-confiture && confiture migrate status --env production"
```

For automated pipelines, add a migration step to your CI/CD workflow that runs before updating the ECS service. See the [Confiture documentation](/confiture/migrate) for more details.
...
aws ecs create-service \
  ...
  --desired-count 3 \
  --launch-type FARGATE \
  --load-balancers "targetGroupArn=arn:aws:elasticloadbalancing:...,containerName=fraiseql,containerPort=8080" \
...
### gRPC and the Application Load Balancer

AWS ALB supports gRPC natively via HTTP/2. When the ALB listener uses HTTPS and the target group protocol version is set to `GRPC`, the ALB routes gRPC traffic (including server streaming) to the Fargate task on port 8080. No separate listener or target group is required for GraphQL and REST — all three transports share the same port:

```bash
# Set the target group protocol version to GRPC for the gRPC listener
aws elbv2 modify-target-group-attributes \
  --target-group-arn arn:aws:elasticloadbalancing:... \
  --attributes Key=deregistration_delay.timeout_seconds,Value=30

# Create a second target group with GRPC protocol version
aws elbv2 create-target-group \
  --name fraiseql-grpc-tg \
  --protocol HTTP \
...
Route gRPC clients to the `fraiseql-grpc-tg` target group (same backend instances) and GraphQL/REST clients to the standard target group. Both point to the same Fargate tasks — FraiseQL handles protocol selection per request.
