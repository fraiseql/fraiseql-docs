...
az containerapp job create \
  ...
  --replica-timeout 300 \
  --trigger-type Manual \
  --args "sh" "-c" "pip install -q fraiseql-confiture && confiture migrate up --env production" \
...
## gRPC and Application Gateway v2

Azure Application Gateway v2 (Standard_v2 and WAF_v2 SKUs) supports gRPC natively. To route gRPC traffic alongside GraphQL and REST traffic through the same Application Gateway to the same FraiseQL backend on port 8080, configure the backend settings to use HTTP/2:

```bash
# Enable HTTP/2 on the Application Gateway (required for gRPC)
az network application-gateway http-settings update \
  --resource-group $RESOURCE_GROUP \
  --gateway-name fraiseql-gateway \
...
# Note: Application Gateway v2 terminates TLS and communicates with
# the backend over HTTP/2 when the backend settings protocol is HTTPS.
# FraiseQL does not need TLS enabled on port 8080 — TLS is terminated
# at the gateway level.
```

For Azure Container Apps (which are used in the SQL Server deployment above), gRPC is supported natively — Container Apps automatically negotiate HTTP/2 with clients, and the traffic reaches FraiseQL's port 8080 over HTTP/2 without additional configuration.
