...
<Aside type="tip">
Confiture is a Python CLI tool — it does not need to run inside the FraiseQL container. Install it via `pip install fraiseql-confiture` (or `uv pip install fraiseql-confiture`) in a lightweight Python image for init containers and migration jobs. Example: `python:3.13-slim` with `pip install fraiseql-confiture` in the entrypoint.
</Aside>

<Aside type="tip" title="Zero-downtime schema updates">
FraiseQL supports hot-reloading schemas via SIGUSR1 or the admin API — in-flight requests are not interrupted. See [Admin API — Schema Reload](/reference/admin-api#schema-reload).
</Aside>
...
`fraiseql run` serves GraphQL, REST, and gRPC from a **single binary on a single port** — no separate processes or containers per transport. **Note:** gRPC requires building FraiseQL with the `grpc-transport` Cargo feature — not included in the official Docker image. See [gRPC Transport](/features/grpc-transport).
