...
<Aside type="note">
The `[analytics]` section configures analytics query definitions and the columnar query cache. It is distinct from the Arrow Flight binary streaming transport, which requires the `arrow` Cargo feature compiled into the binary. See [Arrow Flight Dataplane](/features/arrow-dataplane) for the low-level streaming transport. If you are using the official `ghcr.io/fraiseql/server:latest` image, check whether the `arrow` feature is compiled in before relying on Flight streaming — the `[analytics]` TOML section itself is always available.
</Aside>
...
## Arrow Flight and API Transports
