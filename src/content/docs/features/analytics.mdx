...
<Aside type="tip">
The `tb_calendar` approach provides a single source of truth for all temporal dimensions. `*_n_days` columns enable weighted averages, and the calendar is computed once rather than per fact row.
</Aside>
...
<CardGrid>
  <Card title="Arrow Flight" icon="rocket">
    [Arrow Flight](/features/arrow-dataplane) — High-performance columnar data access
  </Card>
