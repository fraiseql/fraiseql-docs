...
{/* source: CHANGELOG.md (v2.3.0 entry — SagaExecutor / SagaCompensator / PostgresSagaStore landed in v2.3.0; federation gateway sub-command was added in the v2.1 line and the `[gateway]` TOML section is documented as Beta in v2.2 — see `src/content/docs/reference/toml-config.mdx:L47`). The v2.0.1 anchor predated the v2.1.0 release baseline. */}
:::caution[Beta feature]
Apollo Federation support is available as a beta feature.
Core federation queries (`_service`, `_entities`) work in production today.
The built-in federation gateway is available as a beta sub-command; full compatibility with Apollo Router is in progress. Saga execution across services is production-ready with crash recovery, health checks, and monitoring (SagaExecutor, SagaCompensator, PostgresSagaStore).
:::
...
# Compound key example — most types just use the default ["id"]
@fraiseql.type(key_fields=["id", "region"])
class RegionalOrder:
    """An order with a compound federation key."""
    id: ID
    region: str
    total: Decimal
...
if __name__ == "__main__":
    fraiseql.export_schema(
        "schema.json",
        federation=fraiseql.Federation(service_name="order-service"),
    )
```
</TabItem>
</Tabs>
...
<Aside type="tip">
The `key_fields` parameter is available on the type decorator/annotation in all 11 SDKs (Python, TypeScript, Go, Java, C#, F#, Elixir, PHP, Ruby, Dart, Rust). The `extends` parameter marks types that extend a type from another subgraph.
</Aside>

### TOML-based (alternative)
