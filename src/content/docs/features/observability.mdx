...
1. **Enable metrics in `fraiseql.toml`**:
   ```toml
   [metrics]
   enabled = true
   endpoint = "/metrics"
...
5. **Verify metrics updated**:
   ```bash
   curl http://localhost:9090/metrics | grep fraiseql_graphql_queries_total
   ```
...
1. Verify metrics are enabled:
   ```bash
   grep -A5 '\[metrics\]' fraiseql.toml
   ```

2. Check FraiseQL logs for startup errors:
   ```bash
   fraiseql run 2>&1 | grep -i metrics
   ```
...
exclude_labels = ["user_id", "request_id", "session_id"]
```
...
1. Verify OTLP endpoint connectivity:
   ```bash
...
2. Check sampling configuration:
   ```toml
   [tracing.sampling]
   ratio = 1.0  # Sample 100% for testing
   ```
