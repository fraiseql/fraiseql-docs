...
<Aside type="note">
**Metrics** are always available at `/metrics` — no configuration needed. **Tracing** is configured via the `[tracing]` TOML section (since v2.1.0) or standard OpenTelemetry environment variables. **Logging** is controlled via `RUST_LOG` (standard Rust env var). See [TOML Configuration](/reference/toml-config#tracing) for the full `[tracing]` reference.
</Aside>
