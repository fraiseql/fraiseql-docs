...
Pagination works identically via REST query parameters. The default path is `/rest/v1`; configure with `[rest]` `path`. Offset-based: `GET /rest/v1/posts?limit=10&offset=20` maps to `posts(limit: 10, offset: 20)`. Relay cursor-based: `GET /rest/v1/posts?first=10&after=<cursor>` maps to `posts(first: 10, after: "<cursor>")`.
