...
Both are enabled with a single flag in `fraiseql.config()`. No manual Connection/Edge/PageInfo types to define.
...
@fraiseql.query
def posts() -> list[Post]:
    return fraiseql.config(sql_source="v_post", auto_params=False)
```
...
CREATE TABLE tb_post (
  pk_post    BIGINT  GENERATED ALWAYS AS IDENTITY PRIMARY KEY,   -- sequential, internal, never in GraphQL
  id         UUID    UNIQUE NOT NULL DEFAULT gen_random_uuid(),  -- public identifier for node(id: ID!)
...
CREATE VIEW v_post AS
  SELECT
    p.id,
    jsonb_build_object(
      'pk_post', p.pk_post,   -- included so FraiseQL can build cursors
      'id',      p.id,
      'title',   p.title
      -- ...
    ) AS data
...
Pagination works identically via REST query parameters. The default prefix is `/rest`; configure with `[fraiseql.rest]` `prefix` to use `/rest/v1` or any other value. Offset-based: `GET /rest/posts?limit=10&offset=20` maps to `posts(limit: 10, offset: 20)`. Relay cursor-based: `GET /rest/posts?first=10&after=<cursor>` maps to `posts(first: 10, after: "<cursor>")`.
...
<CardGrid>
  ...
  <Card title="Custom Queries" icon="puzzle">
    [Custom Queries](/guides/custom-queries) — Filtering, search, and pagination in practice
  </Card>
