...
<Aside type="note">
Rate limiting in rc.13 applies a global token bucket to all requests, per-endpoint limits to auth routes, and a per-authenticated-user limit (default: 10× the global rate). The rate limiter is in-memory per process — distributed Redis coordination for multi-replica deployments is planned for a future release.
</Aside>
...
The current implementation is **in-memory per instance**. When running multiple FraiseQL instances behind a load balancer, each instance maintains its own token bucket independently — a client can send requests across instances without hitting a shared limit.

A Redis backend is planned for a future release. If you configure `redis_url` today, the server logs a warning and falls back to in-memory:

```toml
[security.rate_limiting]
enabled  = true
# redis_url = "redis://localhost:6379"  # Not yet enforced — logs a warning
```

For production deployments that require distributed rate limiting before the Redis backend ships, apply rate limiting at your load balancer or API gateway layer.
...
- Apply rate limiting at the load balancer layer instead, until the Redis backend ships
