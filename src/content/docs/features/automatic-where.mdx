...
Auto-generated WHERE filters are also available via REST query parameters. For example, if a field `name` has a `contains` filter, you can use `GET /rest/v1/users?name_contains=alice` (the default path is `/rest/v1`; configure with `[fraiseql.rest]` `path`) — equivalent to `users(where: { name: { contains: "alice" } })` in GraphQL.
