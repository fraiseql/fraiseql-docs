...
Auto-generated WHERE filters are also available via REST query parameters. For example, if a field `name` has a `contains` filter, you can use `GET /rest/users?name_contains=alice` (the default prefix is `/rest`; configure with `[fraiseql.rest]` `prefix`) — equivalent to `users(where: { name: { contains: "alice" } })` in GraphQL.
