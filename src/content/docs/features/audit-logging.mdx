...
**Schema (operator-managed):** FraiseQL writes to this table but does not create it. Apply the DDL below in your own migration tooling — the framework does not run migrations against your database.
...
### Pluggable Audit Exporters

<Aside type="note">
Available since v2.1.0. Requires the `audit-syslog` or `audit-webhook` Cargo feature flags. The PostgreSQL audit logger remains the default — exporters run **in addition** to the database backend, streaming entries to external immutable stores for SOC 2/PCI compliance.
</Aside>

FraiseQL supports pluggable audit export sinks that run alongside the PostgreSQL backend:

| Exporter | Feature flag | Protocol | Use case |
|----------|-------------|----------|----------|
| Syslog | `audit-syslog` | RFC 5424 UDP/TCP | SIEM integration (Splunk, Datadog) |
| Webhook | `audit-webhook` | HTTPS POST (batched, with retry) | S3 via Lambda, Loki, custom endpoints |

Enable via Cargo feature flags when building FraiseQL:

```bash
# Enable syslog exporter
cargo build --release --features audit-syslog

# Enable webhook exporter
cargo build --release --features audit-webhook

# Both
cargo build --release --features audit-syslog,audit-webhook
```

The exporters are configured programmatically via `AuditLogger::with_exporters()` at server startup. See the [production security checklist](https://github.com/fraiseql/fraiseql/blob/d0a4ed4ec1770c70707f68fd9019f2b561d87461/docs/guides/production-security-checklist.md) for S3/Splunk/Loki integration patterns.
...
- [Authentication](/building/authentication) - JWT, OIDC, and the `[security.*]` compile-step indirection
- [Encryption](/features/encryption) - Protect audit data
- [Deployment](/operations/deployment-guide) - Production audit setup