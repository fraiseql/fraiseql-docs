...
### Syslog / File / Multiple Backends

<Aside type="caution">
The `[audit.syslog]`, `[audit.file]`, and multi-backend configuration below are **unverified TOML namespaces** and will cause a parse error. The current confirmed configuration is `[security.enterprise]` only (see top of page).

To forward audit events to syslog or files, write a PostgreSQL trigger or use your log aggregator (Fluent Bit, Logstash) to tail the `ta_audit_log` table and forward entries.
</Aside>
