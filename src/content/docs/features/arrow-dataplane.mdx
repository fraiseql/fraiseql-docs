...
| **Port** | 8080 (default) | 8081 (default) |
...
async function queryArrowFlight(
    ...
    const client = new FlightClient({ address: 'grpc://localhost:8081' });
...
1. **Start FraiseQL with Arrow enabled**:
   ...
   ```
   INFO: Arrow Flight dataplane listening on 0.0.0.0:8081
   ```
...
1. Verify Arrow is enabled in `fraiseql.toml`:
   ```toml
   [arrow]
   enabled = true
   ```
...
3. Check FraiseQL logs for startup errors
