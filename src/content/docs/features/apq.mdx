...
<Aside type="note">
APQ is **disabled by default**. Enable it in `fraiseql.toml` with `[graphql.apq] enabled = true`. For multi-instance deployments, a Redis backend is required so all instances share the same query cache (see [Cache Backend](#cache-backend) below).
</Aside>
...
<Aside type="caution">
When running multiple FraiseQL instances (e.g., behind a load balancer), you **must** use `backend = "redis"`. With the default in-memory backend, a client hitting instance A may cache a query that instance B has never seen, causing spurious `PersistedQueryNotFound` errors on retries.
</Aside>
