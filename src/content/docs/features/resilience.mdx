...
| `connect_timeout_ms` | 30 | Connection and query timeout |
...
<Aside type="tip">
The optimal pool size depends on your database server's connection limit and the number of FraiseQL instances. If you run 5 FraiseQL replicas with `pool_max = 20`, you need 100 database connections available. Monitor your database's `max_connections` setting.
</Aside>
...
When a query exceeds the configured `connect_timeout_ms`:
...
3. Increase `pool_min` / `pool_max` if timeouts are caused by connection wait times
...
| `POOL_EXHAUSTED` | All connections in use | Wait and retry, or increase pool_max |
...
- [ ] Configure appropriate `pool_min` / `pool_max` for your workload
- [ ] Set `connect_timeout_ms` based on your slowest acceptable query
...
Increase `pool_min` / `pool_max` or reduce query concurrency:

```toml
[database]
pool_max = 30  # increase from default 20
```
...
Health check endpoints (`/health`, `/ready`) serve all transports — they report the health of the FraiseQL binary regardless of which transports are active. Graceful shutdown drains all in-flight connections across all transports before exiting.
