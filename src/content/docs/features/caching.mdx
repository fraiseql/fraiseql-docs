...
FraiseQL supports response caching for GraphQL queries. Caching can reduce database load dramatically for read-heavy workloads. Caching is configured globally in `fraiseql.toml` under the `[caching]` section.

<Aside type="note">
Caching is disabled by default. Enabling it requires an explicit `[caching]` block in `fraiseql.toml` with `enabled = true`. Until that block is present, every GraphQL request goes directly to the database with no caching applied.
</Aside>
...
3. **Start FraiseQL**
   ...
   On startup, FraiseQL connects to Redis and logs:

   ```
   [caching] backend: redis (redis://localhost:6379)
