...
Client sends request → FraiseQL checks cache → cache miss → query database → store in cache → return result.
...
|-------------|----------|
| 0 - 300s (TTL) | Serve from cache (fresh) |
| 300 - 360s (SWR window) | Serve stale, refresh in background |
| > 360s (Expired) | Fetch fresh, then serve |
...
Target: greater than 80% hit rate for read-heavy workloads.
...
<Aside type="tip">
Redis cache is shared across instances, survives restarts, and provides better eviction policies than in-memory caching.
</Aside>

### Monitor Invalidation Rate

High invalidation rate indicates over-caching mutable data, too-aggressive cascade rules, or ineffective caching:
...
<CardGrid>
  ...
  <Card title="Deployment" icon="document">
