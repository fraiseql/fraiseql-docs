...
2. **Configure the `[caching]` block**
   ...
   <Aside type="caution">
   The `[caching]` TOML section shown here is unverified — it is not in the confirmed section list from `reference/toml-config.mdx`. If this section does not exist, the configuration block will be silently ignored or cause a parse error. Consult the [TOML Configuration Reference](/reference/toml-config) for verified section names.
   </Aside>
...
<Aside type="caution">
The `[caching]` TOML namespace has not been verified against the compiled binary source. If `fraiseql run` rejects this config with an "unknown field" error, enable caching via the `FRAISEQL_CACHE_ENABLED=true` environment variable instead and file an issue.
</Aside>
