...
<Aside type="note">
Even when cascade is disabled for clients, the server still uses cascade data internally for cache invalidation when the SQL function returns it. The toggle only controls GraphQL response exposure.
</Aside>
