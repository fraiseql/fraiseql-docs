...
<Aside type="tip">
This page is the reference for mutation authoring — `mutation_response`, `fn_*` patterns, decorator parameters, transaction boundaries, error envelopes, and observer integration. For a step-by-step introduction that wires up your first mutation end-to-end, see [Adding Mutations](/getting-started/adding-mutations).
</Aside>
...
Protect mutations using role-based access control via the `requires_role` parameter on `@fraiseql.mutation`, or restrict individual fields with `fraiseql.field(requires_scope=...)`. Authentication token validation is configured at the server level via the direct-TOML `[auth]` section (OIDC) or `[auth_hs256]` section (HS256 testing mode) — both auto-wired on `ServerConfig`. See [Authentication](/building/authentication) for the canonical configuration, mutually exclusive section rules, and the security caveats that LEAD that page.
...
4. FraiseQL reads the `entity` JSONB column from the `mutation_response` row and shapes the GraphQL response. If the mutation has [cascade enabled](/features/function-shapes#graphql-cascade-advanced-mutations-with-side-effects) and the function returned cascade data, it is included in the response and fed into the cache invalidation pipeline.
...
<CardGrid>
  ...
  <Card title="Observers" icon="setting">
    [React to mutations with webhooks and side effects](/features/observers)
  </Card>
  <Card title="Authentication" icon="seti:lock">
    [Configure token validation and scope enforcement](/building/authentication)
  </Card>
  <Card title="Testing" icon="approve-check">
    [Test PostgreSQL functions and the full mutation lifecycle](/building/testing)
  </Card>
</CardGrid>
