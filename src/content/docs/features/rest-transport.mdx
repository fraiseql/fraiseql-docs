...
- Keys expire after 24 hours by default. Configure via `idempotency_ttl_seconds` in `[fraiseql.rest]`:
  ```toml
  [fraiseql.rest]
...
<Aside type="note">
For multi-replica deployments, enable the `redis-apq` Cargo feature and set `REDIS_URL` to share idempotency state across replicas. Without Redis, idempotency is per-process only.
</Aside>
...
Per-query cache TTL can be set via `cache_ttl_seconds` in `fraiseql.config()`. The default TTL is configurable via `default_cache_ttl` in `[fraiseql.rest]` (default: 60 seconds).
