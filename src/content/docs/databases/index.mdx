...
All databases support all three transports — GraphQL, REST, and gRPC.

Before picking a database, check the **[Compatibility Matrix](/databases/compatibility)** to see which features are supported on each backend.
...
|-----------------|-----------|-------|--------|-----------|
| **Relay cursor pagination** | ✅ | ✅ | ❌ offset only | ✅ |
| **Offset pagination** | ✅ | ✅ | ✅ | ✅ |
| **Rich filters** | ✅ | ✅ | ✅ | ✅ |
...
<Aside type="note">
SQL Server is a first-class FraiseQL runtime target. If your organization already runs SQL Server for compliance, licensing, or organizational standards, there is no need to migrate to PostgreSQL. FraiseQL compiles your schema definitions into a runtime configuration and executes against your existing T-SQL views and stored procedures — no database migration required. See the [SQL Server Enterprise Guide](/databases/sqlserver-enterprise) for Azure AD, Always On, and compliance patterns.
</Aside>

<Aside type="tip">
FraiseQL schemas are database-agnostic. The `tb_*`, `v_*`, `fn_*`, `tv_*` naming conventions and the data column pattern work across PostgreSQL, MySQL, and SQL Server. SQLite supports read operations only — mutations via `fn_*` functions are not supported on SQLite.
</Aside>
...
- **SQL schema files are database-specific** — tables, views, and functions must be written for each database dialect (PostgreSQL JSONB vs MySQL JSON vs SQL Server FOR JSON PATH differ substantially)
