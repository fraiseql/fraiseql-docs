...
| **Full-Text Search** | Native | Native | Not exposed by FraiseQL | Native |
...
| **Full-text search** | ✅ | ⚠️ Limited | ❌ | ⚠️ CONTAINS/FREETEXT |
| **Mutations** | ✅ | ✅ | ✅ DirectSql | ✅ |
| **Row-level security** | ✅ native RLS | via views | via views | ✅ native RLS |
| **Subscriptions** | ✅ | ⚠️ Polling bridge | ❌ | ⚠️ Polling bridge |
...
<Aside type="tip">
FraiseQL schemas are database-agnostic. The `tb_*`, `v_*`, `fn_*`, `tv_*` naming conventions and the data column pattern work across all four backends. SQLite uses DirectSql for mutations (INSERT/UPDATE/DELETE with RETURNING) instead of stored procedures.
</Aside>
