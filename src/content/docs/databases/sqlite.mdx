...
<Aside type="note">
SQLite uses `MutationStrategy::DirectSql` for mutations. Since SQLite has no stored procedures, FraiseQL executes INSERT, UPDATE, and DELETE statements directly with a RETURNING clause. Mutations decorated with `@fraiseql.mutation` work — the runtime automatically uses DirectSql dispatch instead of function calls.
</Aside>
