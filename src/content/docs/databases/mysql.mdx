...
FraiseQL generates transport-aware views for MySQL:
- JSON-shaped views (GraphQL + REST) use `JSON_ARRAYAGG(...)`
- Row-shaped views (gRPC) use standard `SELECT` with typed columns

Both view types work with all three transports. MySQL supports all three transports equally.
