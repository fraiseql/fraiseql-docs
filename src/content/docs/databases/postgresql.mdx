...
description: PostgreSQL implementation guide for FraiseQL's CQRS architecture with Trinity pattern, JSONB projections, materialized views, and mutation responses.
---
...
### Trinity Pattern: UUID + BIGINT PKs

FraiseQL uses a three-identifier system per entity:

```sql
CREATE TABLE tb_user (
  pk_user    BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,  -- Internal, fast FKs
  id         UUID   NOT NULL UNIQUE DEFAULT gen_random_uuid(), -- Public, exposed in GraphQL
  identifier TEXT   NOT NULL UNIQUE,                           -- Human-readable key (e.g. email)
...
CREATE TABLE tb_post (
  ...
  identifier TEXT   NOT NULL UNIQUE,                           -- Human-readable key (e.g. slug)
...
- `identifier` (TEXT): Human-readable domain key (email, slug, SKU, etc.)
...
CREATE OR REPLACE FUNCTION fn_get_pk_user(p_user_id UUID)
RETURNS BIGINT
...
CREATE OR REPLACE FUNCTION fn_get_user_id(p_pk_user BIGINT)
RETURNS UUID
LANGUAGE SQL STABLE PARALLEL SAFE
AS $$
...
CREATE TYPE mutation_response AS (
  status          TEXT,              -- "success", "failed:validation", "conflict:duplicate_email"
  message         TEXT,              -- Human-readable error/success message
  entity_id       UUID,              -- UUID of created/updated entity
...
```sql
CREATE OR REPLACE FUNCTION sync_tv_user_single(p_user_id UUID) RETURNS VOID
LANGUAGE plpgsql AS $$
...
```sql
-- File: 03311_fn_create_user.sql
CREATE OR REPLACE FUNCTION app.fn_create_user(
...
BEGIN
  ...
  RETURN core.fn_create_user(input_tenant_id, input_user_id, v_input, input_payload);
...
```sql
CREATE OR REPLACE FUNCTION core.fn_create_user(
  input_tenant_id UUID,
  input_user_id UUID,
...
BEGIN
  ...
  INSERT INTO tb_user (id, identifier, email, name, status, created_by)
  VALUES (v_user_id, input_data.email, input_data.email, input_data.name, COALESCE(input_data.status, 'active'), input_user_id)
  RETURNING pk_user INTO v_user_pk;

  SELECT data INTO v_entity_data FROM v_user WHERE id = v_user_id;

  PERFORM sync_tv_user_single(v_user_id);

  RETURN ('success', 'User created successfully', v_user_id, 'User',
...
SELECT COALESCE(
  ...
  '[]'::jsonb
) as posts FROM tb_post p GROUP BY p.fk_user;
```
...
FraiseQL's compiler generates two types of views for PostgreSQL:

**JSON-shaped views** (used by GraphQL and REST):
```sql
-- Example: v_post (generated by compiler)
CREATE VIEW v_post AS
SELECT
    json_agg(row_to_json(p)) AS data
FROM posts p;
```
PostgreSQL produces JSON directly. The server passes it through without parsing.

**Row-shaped views** (used by gRPC):
```sql
-- Example: rv_post (generated by compiler for gRPC)
CREATE VIEW rv_post AS
SELECT id, title, author_id, created_at FROM posts;
```
Standard typed columns. The server maps each column to a protobuf field. No `json_agg` overhead.

PostgreSQL's `json_agg` function has measurable CPU overhead — it's string concatenation under the hood. The row-shaped path avoids this entirely, which is why gRPC can be more efficient for the same query.
