...
**Fix Unbounded Cache**:
```python
# Set TTL to prevent indefinite growth — use cache_ttl_seconds in fraiseql.config()
@fraiseql.query
def expensive_query() -> ExpensiveResult:
    return fraiseql.config(sql_source="v_expensive_query", cache_ttl_seconds=3600)

# Or use LRU cache with max size for non-FraiseQL computations
...
@fraiseql.query
def stable_data() -> StableData:
    return fraiseql.config(sql_source="v_stable_data", cache_ttl_seconds=86400)
```
...
FraiseQL Python mutations are compile-time schema descriptors only — there is no Python runtime execution. Background work (such as sending emails) should be handled by the Rust runtime's observer/NATS system, configured in `fraiseql.toml`. The mutation itself maps to a SQL function:

```python
# CORRECT: Compile-time descriptor — no runtime logic in Python
...
```toml
# Configure observers in fraiseql.toml to trigger async side-effects
[observers]
backend = "nats"
...
```bash
# Before optimization
curl -s http://localhost:8080/graphql -d '{"query":"..."}' | jq '.timing'
...
For high-throughput data export or analytics workloads using the gRPC transport, use row-shaped views (`rv_*`) instead of the JSONB-shaped `v_*` views. Row-shaped views return typed columns directly, avoiding JSON serialization overhead:

```sql
-- Row-shaped view for gRPC (typed columns, no JSONB)
CREATE VIEW rv_user AS
SELECT
    u.id::text        AS id,
    u.email           AS email,
    u.name            AS name,
...
FraiseQL maps `rv_*` views to Protobuf messages at compile time. For large result sets returned over gRPC server streaming, the row-shaped approach reduces per-row serialization cost compared to parsing JSONB.
