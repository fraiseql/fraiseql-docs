...
<Aside type="note">
FraiseQL requires **MySQL 8.0 or later** (or **MariaDB 10.4+**). Key differences from MySQL 5.7:
- `JSON_TABLE()` and JSON path operators (`->`, `->>`) require MySQL 8.0+
- `InnoDB` is required (MyISAM is not supported — no transactions)
- `utf8mb4` charset is required for full Unicode support
</Aside>
...
## FraiseQL Configuration for MySQL
...
# MySQL charset/collation is set in the DATABASE_URL connection string
```
