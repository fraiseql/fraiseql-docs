...
A: No. While helpful, FraiseQL teaches GraphQL concepts gradually. Start with the [5-Minute Quickstart](/getting-started/five-minute-quickstart) — it works even without GraphQL knowledge.
...
A: Yes. FraiseQL respects database transactions — each mutation runs in a PostgreSQL transaction. For distributed workflows spanning multiple services, you can implement the saga pattern in your business logic using a series of FraiseQL mutations with compensating transactions.
...
A: Yes, but not by default. You can add caching with `cache_ttl_seconds=` on `@fraiseql.query` or use an external cache (Redis).
...
A: Role-Based Access Control (RBAC) with scopes. Use `fraiseql.field(requires_scope=...)` on type fields to enforce field-level permissions. See [advanced patterns](/guides/advanced-patterns).
...
A: FraiseQL supports per-endpoint rate limits configured in `fraiseql.toml` under `[security.rate_limiting]`. Override at runtime with environment variables such as `RATE_LIMIT_AUTH_START`.
...
A: Enable `RUST_LOG=debug`, check database query count (should be < 5 per request), add indexes, analyze with EXPLAIN. See [performance troubleshooting](/troubleshooting/performance-issues).
...
A: [Discord](https://discord.gg/fraiseql) for quick questions, [GitHub Issues](https://github.com/fraiseql/fraiseql/issues) for deep dives, [Troubleshooting Guide](/troubleshooting) for solutions.
...
A: Yes. FraiseQL serves REST, GraphQL, and gRPC from the same binary. Add `rest_path` and `rest_method` annotations to expose operations as REST endpoints alongside GraphQL.
...
**Can I serve REST and GraphQL from the same binary?**
Yes. FraiseQL serves all three transports (GraphQL, REST, gRPC) simultaneously from the same binary on port 8080. No separate processes needed.

**Do I need to choose between transports?**
No. Add `rest_path`/`rest_method` annotations to expose operations as REST. gRPC endpoints are auto-generated for all operations when `[grpc]` is enabled — see [gRPC Transport](/features/grpc-transport). Operations without REST annotations are GraphQL-only (plus gRPC if enabled).

**Does gRPC require a separate port?**
No. gRPC runs on the same port as GraphQL and REST. HTTP/2 content-type negotiation distinguishes gRPC requests (`application/grpc`) from HTTP/1.1 REST and GraphQL.

**Is REST auto-generated from my schema?**
No. REST endpoints require explicit `rest_path` and `rest_method` annotations. This is intentional — you control which operations are exposed as REST and at what paths.

**Can I use REST without GraphQL?**
Yes, but GraphQL introspection remains available on all deployments (unless disabled with `fraiseql run --no-introspection`). REST and GraphQL are independent transports.
...
<CardGrid>
  ...
  <Card title="GitHub Issues" icon="github">
    Deep-dive questions, feature requests, and RFCs. [Open an issue](https://github.com/fraiseql/fraiseql/issues)
  </Card>
