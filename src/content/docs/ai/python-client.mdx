...
FraiseQL also exposes REST endpoints at `/rest/v1/<resource>` (the default path is `/rest/v1`; configure with `[fraiseql.rest]` `path`). You can query these directly from Python scripts using `httpx` or `requests` without the `FraiseQLClient`:
