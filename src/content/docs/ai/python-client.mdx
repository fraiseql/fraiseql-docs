...
FraiseQL also exposes REST endpoints at `/rest/<resource>` (the default prefix is `/rest`; configure with `[fraiseql.rest]` `prefix` to use `/rest/v1` or any other value). You can query these directly from Python scripts using `httpx` or `requests` without the `FraiseQLClient`:
