...
| `rest_path` | `str \| None` | `None` | REST endpoint path template (e.g. `"/users/{id}"`). Path parameter names must match function argument names. Defaults to `None` (no REST endpoint). |
| `rest_method` | `str \| None` | `"GET"` | HTTP method for the REST endpoint. Defaults to `"GET"` for queries. |
| `grpc_service` | `str \| None` | `None` | gRPC service name (e.g. `"UserService"`). Defaults to `None` (no gRPC endpoint). |
| `grpc_method` | `str \| None` | `None` | gRPC method name (e.g. `"GetUser"`). Required if `grpc_service` is set. |
...
#### REST and gRPC annotation examples

<Aside type="note">
REST (`rest_path`, `rest_method`) and gRPC (`grpc_service`, `grpc_method`) transport annotations require the `rest-transport` and `grpc-transport` Cargo features respectively, and the corresponding `[fraiseql.rest]` / `[fraiseql.grpc]` TOML sections. Available since rc.15.
</Aside>

<Tabs syncKey="language">
  <TabItem label="Python">
    ```python
    @fraiseql.query(rest_path="/users/{id}", rest_method="GET", grpc_service="UserService", grpc_method="GetUser")
...
| `rest_path` | `str \| None` | `None` | REST endpoint path template (e.g. `"/users"`). Path parameter names must match function argument names. |
| `rest_method` | `str \| None` | `"POST"` | HTTP method for the REST endpoint. Defaults to `"POST"` for mutations. |
| `grpc_service` | `str \| None` | `None` | gRPC service name (e.g. `"UserService"`). Defaults to `None` (no gRPC endpoint). |
| `grpc_method` | `str \| None` | `None` | gRPC method name (e.g. `"CreateUser"`). Required if `grpc_service` is set. |
...
| `@query` | Define query | `sql_source`, `auto_params`, `cache_ttl_seconds`, `rest_path`, `grpc_service` |
| `@mutation` | Define mutation | `sql_source`, `operation`, `requires_scope`, `rest_path`, `grpc_service` |
