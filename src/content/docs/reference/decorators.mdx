...
<Aside type="note">
REST (`rest_path`, `rest_method`) and gRPC (`grpc_service`, `grpc_method`) transport annotations require the `rest-transport` and `grpc-transport` Cargo features respectively, and the corresponding `[fraiseql.rest]` / `[fraiseql.grpc]` TOML sections. Available since v2.1.
</Aside>
