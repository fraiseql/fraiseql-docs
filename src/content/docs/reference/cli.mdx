...
| `--rest-path <PATH>` | `"/rest/v1"` | REST URL path prefix. Overrides `[fraiseql.rest].path` in `fraiseql.toml`. Requires the `rest-transport` feature. |
...
Start a federation gateway that routes queries to multiple FraiseQL subgraphs.

```bash
fraiseql gateway [OPTIONS]
```
...
| Option | Default | Description |
|--------|---------|-------------|
| `--config` | `./gateway.toml` | Path to `gateway.toml` |
| `--port` | `4000` | Gateway listen port |
| `--bind` | `0.0.0.0` | Bind address |
| `--subgraph` | — | Add subgraph (`name=url`), repeatable |
...
fraiseql gateway \
  --port 4000 \
  --subgraph inventory=http://localhost:4001 \
  --subgraph billing=http://localhost:4002
```
...
Run a comprehensive health check of your FraiseQL project. Validates configuration, schema, database connectivity, secrets, and coherence between features. Use this as the first diagnostic step when something isn't working.
...
| Option | Default | Description |
|--------|---------|-------------|
| `--config <PATH>` | `fraiseql.toml` | Path to configuration file |
| `--schema <PATH>` | `schema.compiled.json` | Path to compiled schema |
| `--database-url <URL>` | `$DATABASE_URL` | Override database connection string |
...
|-------|----------|-------------|
| Schema file exists | Fail | Compiled schema file is present |
| Schema parses | Fail | Schema JSON is valid with types/queries/mutations |
| Schema format version | Fail | Schema version field is `v1` |
| fraiseql.toml found | Warn | Configuration file exists |
| TOML syntax valid | Fail | Configuration file parses without errors |
| DATABASE_URL set | Fail | Database connection string is provided |
| DATABASE_URL reachable | Fail | TCP connection to database succeeds (5s timeout) |
| FRAISEQL_JWT_SECRET | Warn | JWT signing secret is set |
| FRAISEQL_REDIS_URL | Pass/Skip | Redis connectivity (skipped if not configured) |
| TLS certificate | Fail | Certificate file exists (if TLS enabled in config) |
| Cache + auth coherence | Warn | Caching without authorization policies |
...
```bash
# Run all checks with default paths
fraiseql doctor

# Check a specific config and schema
fraiseql doctor --config deploy/fraiseql.toml --schema build/schema.compiled.json
...
```json
[
  { "name": "Schema file exists", "status": "pass", "detail": "schema.compiled.json found", "hint": null },
  { "name": "FRAISEQL_JWT_SECRET", "status": "warn", "detail": "Not set", "hint": "Set FRAISEQL_JWT_SECRET for JWT authentication" }
]
```
...
FraiseQL does not have a `fraiseql migrate` subcommand. Database migrations are managed by **Confiture**, the companion migration tool:

```bash
confiture migrate up                         # Apply pending migrations
confiture migrate down                       # Roll back the last migration
confiture migrate status                     # Show migration status
confiture migrate create add_user_table      # Create a new migration file
```

See the [Confiture documentation](/confiture/migrate) for full migration reference.
...
| Start federation gateway | `fraiseql gateway --config gateway.toml` |
...
| Run migrations | `confiture migrate up` |
...
confiture migrate up                   # Apply migrations (Confiture, not fraiseql)
