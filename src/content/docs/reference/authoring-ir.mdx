...
// ⚠️ Accepted but deprecated — ! suffix is stripped, nullable inferred as false
{ "name": "id", "type": "ID!" }
```

<Aside type="caution">
The `!` suffix (e.g., `"type": "ID!"`) is accepted for compatibility with GraphQL conventions: the parser strips the `!` and sets `nullable = false`. However, this form is **deprecated** and emits a warning at compile time. Always prefer the explicit `"type": "ID", "nullable": false` form. If both `!` and an explicit `nullable` field are present, the explicit `nullable` value wins.
</Aside>
...
<Aside type="note">
Prior to the fix for [#121](https://github.com/fraiseql/fraiseql/issues/121), using `"type": "ID!"` produced `references unknown type 'ID!'`. This is now accepted with a deprecation warning. Use `"type": "ID", "nullable": false` to avoid the warning.
</Aside>
