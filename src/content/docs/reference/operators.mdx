...
<Aside type="note">
Rich type filter operators use `camelCase` naming **without** an underscore prefix — unlike basic operators (`_eq`, `_in`, etc.). For example: `domainEq`, `countryCodeIn`, `distanceWithin`. Using underscore-prefixed names for these operators will produce a GraphQL validation error.
</Aside>
...
|----------|-------|---------------|
| **Contact** | Email | `domainEq`, `domainIn`, `domainEndswith`, `localPartStartswith` |
| | PhoneNumber | `countryCodeEq`, `countryCodeIn`, `isValid`, `typeEq` |
| | URL | `protocolEq`, `hostEq`, `pathStartswith` |
| | DomainName | `tldEq`, `tldIn` |
| **Geography** | CountryCode | `eq` |
| | Coordinates | `distanceWithin` |
| **Financial** | IBAN | `countryEq`, `checkDigitEq` |
| | VIN | `wmiEq` |
| | TrackingNumber | `carrierEq` |
