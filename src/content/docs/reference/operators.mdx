...
<Aside type="note">
`_nlike`, `_nilike`, `_regex`, `_iregex`, `_nregex`, and `_niregex` are **not yet available** — they are defined in the operator catalog but not wired in the runtime parser. Tracked in [issue #51](https://github.com/fraiseql/fraiseql/issues/51). Until fixed, use `_like` with a negating `_not` wrapper, or filter in application code.
</Aside>
...
input StringFilter {
    ...
    # _nlike, _nilike, _regex, _iregex, _nregex, _niregex: not yet wired (issue #51)
    _is_null: Boolean
}
```
