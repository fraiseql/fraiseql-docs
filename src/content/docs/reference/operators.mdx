...
| `_ilike` | Pattern match (case-insensitive) | `ILIKE $pattern` | **PostgreSQL only** |
| `_nlike` | Not like | `NOT LIKE $pattern` | All |
| `_nilike` | Not ilike | `NOT ILIKE $pattern` | **PostgreSQL only** |
| `_regex` | Regular expression | `~ $pattern` | **PostgreSQL only** |
| `_iregex` | Case-insensitive regex | `~* $pattern` | **PostgreSQL only** |
| `_nregex` | Not regex | `!~ $pattern` | **PostgreSQL only** |
| `_niregex` | Not iregex | `!~* $pattern` | **PostgreSQL only** |

<Aside type="note">
Operators marked **PostgreSQL only** are not available in MySQL, SQLite, or SQL Server. Using them with a non-PostgreSQL database will result in a compilation error.
</Aside>
...
input StringFilter {
    ...
    _nlike: String         # NOT LIKE
    _nilike: String        # NOT ILIKE (PostgreSQL only)
    _regex: String         # PostgreSQL ~ operator
    _iregex: String        # PostgreSQL ~* operator (PostgreSQL only)
    _nregex: String        # PostgreSQL !~ operator (PostgreSQL only)
    _niregex: String       # PostgreSQL !~* operator (PostgreSQL only)
    _is_null: Boolean
}
```
