...
<Aside type="note">
Looking for **semantic scalar types** with advanced filtering capabilities? See [Semantic Scalars](/reference/semantic-scalars) for 50+ specialized types like `Email`, `Coordinates`, `IBAN`, and more, each with domain-specific operators and validation rules.
</Aside>
...
FraiseQL provides **50+ domain-specific scalar types** with built-in validation, organized into logical categories.
...
**Filter Operators** (camelCase, no underscore prefix):

| Operator | Description | Example |
|----------|-------------|---------|
| `domainEq` | Match domain exactly | `"acme.com"` |
| `domainIn` | Domain in list | `["acme.com", "corp.com"]` |
| `domainEndswith` | Domain suffix | `".edu"` |
| `localPartStartswith` | Local part prefix | `"sales."` |

```graphql
query {
    # Find company domain emails
    leads(where: { email: { domainEq: "acme.com" } }) { id email }

    # All .edu domains
    students(where: { email: { domainEndswith: ".edu" } }) { id email }
}
```
...
| Operator | Description | Example |
|----------|-------------|---------|
| `eq` | Exact country code match | `"US"` |
...
```graphql
query {
    # EU customers with company domain emails
    customers(where: {
        country: { eq: "DE" }
        email: { domainEndswith: ".de" }
    }) {
...
For scalars with custom validation, inherit from `CustomScalar` and implement `serialize`, `parse_value`, and `parse_literal`:
...
- [Type System](/concepts/type-system) — Complete type reference, including Protobuf type mappings for gRPC
