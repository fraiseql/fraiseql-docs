...
CREATE TABLE tf_sales (
    ...
    -- Denormalized UUID copies for analytics filtering (not foreign keys — intentionally bypasses the FK pattern for analytics performance)
...
<Aside type="note">
Fact table views (`tf_`/`v_sales`) do not follow the standard `id, data` view shape. Fact tables are analytics-only and not directly exposed as GraphQL entities — they use `pk_` for internal joins and return custom JSONB shapes for aggregation queries.
</Aside>
...
Foreign keys reference internal primary keys. The FK type must match the PK type for the target database:
...
For junction tables (PostgreSQL):
...
All mutation functions return the `mutation_response` composite type:

```sql
-- Define once per database
CREATE TYPE mutation_response AS (
    status         TEXT,       -- 'success', 'failed:reason', 'conflict:reason'
    message        TEXT,
    entity_id      UUID,
    entity_type    TEXT,
    entity         JSONB,
...
CREATE FUNCTION fn_create_user(...) RETURNS mutation_response
CREATE FUNCTION fn_update_user(...) RETURNS mutation_response
CREATE FUNCTION fn_delete_user(...) RETURNS mutation_response
CREATE FUNCTION fn_publish_post(...) RETURNS mutation_response
```
...
CREATE FUNCTION sync_tv_user_single(p_id UUID) RETURNS VOID
```
...
REST path annotations (`rest_path`) follow standard REST URL conventions: lowercase, hyphens for spaces, plural nouns for collections (e.g., `/users`, `/blog-posts/{id}`).
...
<CardGrid>
  ...
  <Card title="Decorators" icon="puzzle">
    [Decorators](/reference/decorators) — Python decorator reference
  </Card>
  <Card title="GraphQL API" icon="information">
    [GraphQL API](/reference/graphql-api) — GraphQL API reference
  </Card>
</CardGrid>
