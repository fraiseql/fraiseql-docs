...
Transport annotations are **optional** on schema operations authored with other SDKs. When using `fraiseql-rust` for Rust-native schema authoring (separate from the authorization library), add `rest_path`, `rest_method`, `grpc_service`, and `grpc_method` to the `#[fraiseql::query]` and `#[fraiseql::mutation]` proc-macro attributes.
...
async fn posts_grpc(limit: Option<i32>) -> Vec<Post> { vec![] }
