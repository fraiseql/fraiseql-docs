...
<Aside type="note">
`fraiseql-rust` is an authorization library, not a schema authoring DSL. Schema authoring is done via the other SDKs — see [SDK Design Philosophy](/concepts/sdk-design). Also includes `fraiseql-client`, an async HTTP client for consuming FraiseQL APIs.
</Aside>
