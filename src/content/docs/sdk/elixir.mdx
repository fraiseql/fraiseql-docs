...
<Aside type="note">
The Elixir SDK v2.1.0 is currently in the `[Unreleased]` section of the changelog — available on the main branch but not yet formally tagged. Check the [FraiseQL releases](https://github.com/fraiseql/fraiseql/releases) page for the stable release.
</Aside>

<Aside type="note">
This is not a GraphQL client library. FraiseQL Elixir lets you **define** APIs, not consume them. For consuming GraphQL APIs, use any standard GraphQL client (e.g. Neuron, AbsintheClient).
</Aside>

<Aside type="tip">
FraiseQL Elixir requires **Elixir 1.14+** and **OTP 25+**. The macros run at compile time — no Elixir code executes at GraphQL query time.
</Aside>
...
FraiseQL maps query arguments directly to `WHERE` clauses on the backing SQL view. An `:is_published` argument becomes `WHERE is_published = $1` — no resolver code needed.
