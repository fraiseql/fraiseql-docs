...
<Aside type="note">
The F# SDK is primarily a **schema authoring SDK** — you define APIs, not consume them. However, since v2.1.0 the SDK also includes `FraiseQLClient`, an async HTTP client for consuming FraiseQL APIs from .NET applications. See [HTTP Client](#http-client) below.
</Aside>
...
<Aside type="note">
Available since v2.1.0. The HTTP client is in the same `FraiseQL.FSharp` NuGet package. It shares the exception hierarchy with the C# SDK.
</Aside>
...
The exception hierarchy (`FraiseQLException`, `GraphQLException`, `AuthenticationException`, `RateLimitException`, `FraiseQLTimeoutException`, `NetworkException`) is identical to the [C# SDK](/sdk/csharp#exception-hierarchy).
