...
Transport annotations (`rest_path`/`rest_method`) let you expose operations as REST endpoints. All 11 SDKs support REST annotations as of v2.1.0. gRPC endpoints are auto-generated when `[grpc]` is enabled — no per-operation annotation needed. See [gRPC Transport](/features/grpc-transport).
