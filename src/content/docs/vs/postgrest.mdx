...
|---------|----------|-----------|
| REST | Annotation-driven | Auto-wired from schema |
| GraphQL | Yes | No |
...
| REST endpoint control | Explicit annotations | Auto-expose all tables/views |
...
<Aside type="note">
FraiseQL's REST transport arrived in rc.15. PostgREST has been production-ready for many years. If you need REST-only from PostgreSQL today, PostgREST is mature and battle-tested. FraiseQL's REST transport is newer and the annotation model requires more upfront work to define your API surface.
</Aside>
