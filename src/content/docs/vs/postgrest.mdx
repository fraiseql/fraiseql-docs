...
| Embedding | Coming soon (Phase 2) | `?select=*,orders(*)` |
| Bulk operations | Coming soon (Phase 2) | Yes |
| Full-text search | Coming soon (Phase 2) | `?fts=term` |
...
<Aside type="note">
FraiseQL's REST transport arrived in rc.15. PostgREST has been production-ready for many years. FraiseQL now has feature parity for core REST operations (filtering, sorting, pagination, field selection, ETags, `Prefer` header), but PostgREST retains advantages in embedding (`?select=*,orders(*)`), bulk operations, and full-text search — these are planned for FraiseQL Phase 2.
</Aside>
