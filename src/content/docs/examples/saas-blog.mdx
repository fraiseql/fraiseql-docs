...
This page walks through a production-ready, multi-tenant blogging platform built on FraiseQL. Every snippet below is the runnable form — copy into your own project, adjust the names, and you have the same patterns in your codebase. If you want a runnable scaffold first, walk the [quickstart](/getting-started/quickstart) and then return here.
...
1. **Scaffold a project**
   ...
   If you don't already have a FraiseQL project, walk the [quickstart](/getting-started/quickstart) — it covers `fraiseql init`, the minimal `fraiseql.toml`, and the first `fraiseql run`.

2. **Install the schema above**

   Apply the SQL from the [Database Schema](#database-schema) section in this page (tables, views, RLS policies, audit trigger). Either via your migration tool or directly:
...
8. **Add tests for your patterns**

   Once the example runs end-to-end, codify the invariants in your own test suite (`pytest`, `vitest`, `go test`, etc.). The patterns to assert:

   - Tenant isolation: a query from tenant A returns 0 rows from tenant B.
   - Soft delete: deleted posts disappear from `posts` but reappear via `restorePost`.
   - Role-based access: a `subscriber` cannot call `createPost`.
   - Live updates: subscribing to `commentCreated` receives a payload after a `createComment` mutation.
...
- [Multi-tenancy guide](/building/multi-tenancy) — row-level security and per-tenant executor isolation
- [Authentication](/building/authentication) — JWT/OIDC, PKCE, and token revocation for production deployments
- [Performance benchmarks](/operations/performance-benchmarks) — throughput and latency numbers measured against this schema
- [Observers](/features/observers) — event-driven side effects (email, webhooks) triggered by mutations
