...
This page walks through a Google Docs-like collaborative editor on FraiseQL with `pg_notify`-driven subscriptions. Every snippet below is the runnable form — copy into your own project, adjust the names, and you have the same pattern in your codebase. If you want a runnable scaffold first, walk the [quickstart](/getting-started/quickstart) and then return here.
...
2. **Install the schema above**
   ...
   Apply the SQL from the [Database Schema](#database-schema) section in this page (tables, views, mutation functions). Either via your migration tool or directly:
...
5. **Open a GraphQL subscription**

   Use the GraphiQL playground at `http://localhost:8080/graphiql` or a WebSocket client (e.g. `wscat -c ws://localhost:8080/ws`). Subscribe to `documentChanged(documentId: "…")`.

6. **Apply an edit**

   In a second tab, issue an `applyEdit` mutation. The subscription tab receives the update via `pg_notify` and the WebSocket dispatch.
</Steps>
...
- [Observers](/features/observers) — event-driven architecture for real-time side effects
- [Authentication](/building/authentication) — JWT/OIDC and session management for collaborative apps
- [CQRS patterns](/concepts/cqrs) — command/query separation for high-throughput write paths
- [SaaS Blog example](/examples/saas-blog) — multi-tenant patterns to combine with real-time features
