...
MUTATION FUNCTIONS:
All mutation functions return mutation_response (the canonical v2 composite type):
  status TEXT, message TEXT, entity_id UUID, entity_type TEXT,
  entity JSONB, updated_fields TEXT[], cascade JSONB, metadata JSONB

- fn_create_<entity>(args...) RETURNS mutation_response
- fn_update_<entity>(p_id UUID, args...) RETURNS mutation_response
- fn_delete_<entity>(p_id UUID) RETURNS mutation_response

Example pattern:
RETURN ROW('success', 'Entity created', v_id, '<Entity>',
    (SELECT data FROM v_<entity> WHERE id = v_id),
...
CREATE TABLE tb_article_tag (
    fk_article BIGINT NOT NULL REFERENCES tb_article(pk_article),
...
Generate:
1. fn_create_<entity>(args...) — insert a row, return mutation_response
2. fn_update_<entity>(p_id UUID, args...) — update by UUID id, return mutation_response
3. fn_delete_<entity>(p_id UUID) — soft delete by setting deleted_at, return mutation_response
...
BEGIN
    ...
    RETURN ROW('success', 'Product created', v_id, 'Product',
        (SELECT data FROM v_product WHERE id = v_id),
        NULL, NULL, NULL)::mutation_response;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    IF v_id IS NULL THEN
        RETURN ROW('failed:not_found', 'product not found: ' || p_id,
            NULL, 'Product', NULL, NULL, NULL, NULL)::mutation_response;
    END IF;

    RETURN ROW('success', 'Product updated', v_id, 'Product',
        (SELECT data FROM v_product WHERE id = v_id),
        NULL, NULL, NULL)::mutation_response;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    RETURN ROW('success', 'Product deleted', p_id, 'Product',
