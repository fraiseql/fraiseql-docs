...
Standard `SELECT` with typed columns is PostgreSQL's native fast path. No string building; the query engine materializes the values it already has in memory. The savings scale with result set size and nesting depth.
...
<Aside type="caution">
This section states what this post does not claim.
</Aside>
...
**Not zero overhead.** The column mapping from PostgreSQL row to protobuf field is fast but not free. There's a thin server-side layer reading the row and writing the protobuf message. It's measurably faster than a JSON round-trip, but the delta is microseconds to low milliseconds, not orders-of-magnitude differences.
...
For GraphQL performance data, see the [performance benchmarks page](/operations/performance-benchmarks). Those numbers cover the GraphQL transport under concurrent load.

## Next steps

- [Performance benchmarks](/operations/performance-benchmarks) — GraphQL transport throughput and latency numbers
- [REST direct execution benchmark](/community/blog/rest-direct-execution-benchmark) — REST vs. GraphQL bridge execution path comparison
- [TOML configuration](/concepts/configuration) — transport-aware settings that affect gRPC and REST paths
- [SaaS Blog example](/examples/saas-blog) — the reference schema for reproducing serialization comparisons
