...
import { CardGrid, Card, Tabs, TabItem, Steps, Aside } from '@astrojs/starlight/components';

<Aside type="caution">
Comparison snapshot as of 2026-05-30 against FraiseQL v2.3.2 (frozen SHA `d0a4ed4e`) and Hasura's then-current stable release. Pricing and feature parity ages out quickly; verify current Hasura state at [hasura.io/pricing](https://hasura.io/pricing).
</Aside>

Both FraiseQL and Hasura provide GraphQL APIs over databases. Here's how they differ.

{/* source: crates/fraiseql-core/src/config/auth.rs:L8 — `pub struct AuthConfig` ([auth] OIDC section) */}
{/* source: crates/fraiseql-cli/src/config/toml_schema/security.rs:L1 — module doc-comment "Security configuration types for `[security.*]` and `[auth]` TOML sections" */}
...
> Pricing information captured 2026-05-30. Verify current pricing at [hasura.io/pricing](https://hasura.io/pricing).
...
{/* source: crates/fraiseql-core/src/config/auth.rs:L8 — `pub struct AuthConfig` ([auth] OIDC) */}
{/* source: crates/fraiseql-server/src/server_config/hs256.rs — `[auth_hs256]` HS256 testing config (mutually exclusive with [auth]) */}
...
<Aside type="caution">
The TOML tab below uses `[[observers.rules]]` as illustrative syntax. The verified production handler syntax is `[[observers.handlers]]` — see the [TOML configuration reference](/reference/toml-config#observers).
</Aside>
...
1. **Install the FraiseQL CLI**
   ...
   ```bash
   # Download the FraiseQL binary from GitHub Releases
   # Visit https://github.com/fraiseql/fraiseql/releases for the latest release and install instructions
...
2. **Convert your Hasura YAML to a FraiseQL schema**
   ...
   For each Hasura table, create a FraiseQL type and a SQL view. See the examples above for the conversion pattern, or refer to the [full migration guide](/building/migrations/from-hasura).
...
For a complete walkthrough including subscription migration and permission replication, see the [Migrating from Hasura](/building/migrations/from-hasura) guide.

Migrating from Hasura? See the [step-by-step migration guide](/building/migrations/from-hasura).
...
<CardGrid>
  ...
  <Card title="Performance Benchmarks" icon="bars">
    See how FraiseQL performs with real numbers. [View Benchmarks](/operations/performance-benchmarks)
  </Card>
  <Card title="Migrate from Hasura" icon="right-arrow">
    Step-by-step guide to moving your existing Hasura setup. [Migration Guide](/building/migrations/from-hasura)
  </Card>
