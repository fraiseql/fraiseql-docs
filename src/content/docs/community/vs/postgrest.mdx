...
<Aside type="caution">
Comparison snapshot as of 2026-05-30 against FraiseQL v2.3.2 (frozen SHA `d0a4ed4e`) and the then-current PostgREST stable release. Verify current PostgREST features at [postgrest.org](https://postgrest.org).
</Aside>
...
{/* source: crates/fraiseql-server/Cargo.toml — `grpc` Cargo feature gates the gRPC binary path; REST is unconditional v2.1+ surface */}
{/* source: sdks/official/fraiseql-python/src/fraiseql/decorators.py — `@fraiseql.query(rest_path=..., rest_method=...)` annotation surface */}
...
| GraphQL | Yes | No |
| gRPC | Yes (requires `grpc` feature flag) | No |
...
| Full-text search | Not exposed on REST surface at v2.3.2 | `?fts=term` |
...
<CardGrid>
  <Card title="Migrate from PostgREST" icon="rocket">
    ...
    [Read guide](/building/migrations/from-postgrest)
  </Card>
</CardGrid>
