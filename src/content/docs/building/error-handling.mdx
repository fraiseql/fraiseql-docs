...
| Error type | GraphQL | REST status | gRPC status |
|-----------|---------|-------------|-------------|
| Validation error | `errors` array | 400 | `INVALID_ARGUMENT` |
| Authentication failure | `errors` array | 401 | `UNAUTHENTICATED` |
| Permission denied | `errors` array | 403 | `PERMISSION_DENIED` |
| Not found | `errors` array | 404 | `NOT_FOUND` |
| Rate limited | `errors` array | 429 | `RESOURCE_EXHAUSTED` |
| Server error | `errors` array | 500 | `INTERNAL` |
