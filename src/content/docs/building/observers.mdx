...
:::note[Page scope]
This page is the **how-to-install** leg of the four-page observers cluster. It covers bootstrapping `tb_observer`, applying `migrations/06_create_observer_management.sql`, registering observers via `POST /api/observers`, and wiring an authenticating reverse proxy in front of the management API. For the **what-it-is** (architecture, action types, security caveats, the FW-20 + FW-21 critical combination), see [Observers (Features)](/features/observers). For the **subscriber-side receiver patterns** (downstream services consuming FraiseQL's outbound webhooks), see [Observer and webhook patterns](/building/observer-webhook-patterns). For the **operating-in-production** runbook (alerts, sizing, recovery), see [Observer operations runbook](/operations/observer-runbook).
:::

:::caution[Stale until next cycle — full rewrite deferred]
This page's body still describes the Python `@observer` decorator + `actions=[slack(), email()]` kwargs, which **do not exist** in the `fraiseql` Python package at v2.3.2. The full rewrite is deferred to a later Phase 03 cycle once FW-13 [#340](https://github.com/fraiseql/fraiseql/issues/340), FW-14 [#341](https://github.com/fraiseql/fraiseql/issues/341), and FW-15 [#342](https://github.com/fraiseql/fraiseql/issues/342) are decided upstream. Until then, **treat the body as aspirational** and use [Observers (Features)](/features/observers) as the source of truth at v2.3.2. The Phase 03 / Cycle 3 scope is the primary page rewrite plus this scope-statement disambiguation across all four observer pages.
:::
...
- [Observers (Features)](/features/observers) — what observers are, action types, security caveats (FW-18 / FW-19 / FW-20 / FW-21 / FW-22), and the F056 atomicity guarantee.
