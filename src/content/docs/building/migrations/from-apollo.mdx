...
FraiseQL serves REST and gRPC alongside GraphQL from the same compiled schema. If you're running Apollo Router as a gateway to serve non-GraphQL clients, FraiseQL eliminates that layer.
...
7. **Performance Test and Decommission**
   ...
   Run performance tests to validate the improvement. Apollo with DataLoader typically runs 10-20 queries per request; FraiseQL uses pre-compiled SQL views, reducing this to 1-2 queries. (These figures are architectural estimates based on typical N+1 patterns. See [Performance Benchmarks](/guides/performance-benchmarks) for measured RPS data.)
...
<CardGrid>
  <Card title="FraiseQL vs Apollo" icon="open-book">
    Side-by-side feature and performance comparison.

    [Read comparison](/vs/apollo)
  </Card>
