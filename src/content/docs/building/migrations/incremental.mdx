...
```bash
# Install FraiseQL — see https://github.com/fraiseql/fraiseql/releases for install instructions
...
Incremental adoption works per-transport too. Start with GraphQL (zero annotations needed), add `rest_path` annotations for operations that need REST exposure, then enable `[grpc]` for service-to-service calls. Each transport layer is additive.
