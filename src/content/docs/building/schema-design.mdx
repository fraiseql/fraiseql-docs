...
<Tabs syncKey="language">
  <TabItem label="Python">
    ```python
    @fraiseql.query   # Reads from view
    def user(id: ID) -> User:
        return fraiseql.config(sql_source="v_user")

    @fraiseql.query   # Reads from materialized table
    def users(limit: int = 20) -> list[User]:
        return fraiseql.config(sql_source="tv_user")
    ```
  </TabItem>
  <TabItem label="TypeScript">
    ```typescript
...
def featured_posts() -> list[Post]:
    return fraiseql.config(sql_source="tv_post")
...
<Aside type="caution">
The `[[observers.handlers]]` fields below (`entity_type`, `on`, `actions`) are not confirmed in the TOML configuration reference. The confirmed fields for `[[observers.handlers]]` are `name`, `event`, `action`, `webhook_url`. See [TOML Configuration](/reference/toml-config#observers) for the verified observer syntax.
</Aside>
...
@fraiseql.query
def posts() -> list[Post]:
    return fraiseql.config(sql_source="posts")  # points at the posts table
```
...
Transport annotations are optional per operation. You can expose some operations via GraphQL only, others via REST + GraphQL, and others via all three transports. Design your schema with the consumer in mind — not every operation needs every transport.
