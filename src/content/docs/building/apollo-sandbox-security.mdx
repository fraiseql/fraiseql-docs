...
**Does it track my data?** No. Your queries and results never leave your browser (except to your configured GraphQL endpoint). Apollo Sandbox has optional telemetry that is opt-in only.

**Is the playground enabled in production?** No. The GraphQL playground is disabled by default in production mode. It is only active in development.
...
- Apollo Sandbox telemetry is opt-in only
...
FraiseQL's server embeds Apollo Sandbox with cookies disabled. The server generates a JavaScript snippet equivalent to:
...
By default:
- No cookies set
- No usage tracking from FraiseQL itself
- No external analytics
- Apollo Sandbox does not send telemetry by default
...
A: Yes. Apollo Sandbox is open source (Apache 2.0 license). See [Apollo Sandbox product docs](https://www.apollographql.com/docs/graphos/platform/sandbox) for details. You can audit the code via Apollo GraphQL's GitHub organisation.

**Q: Can we use GraphiQL instead for compliance?**
A: Yes. FraiseQL supports both. Configure the playground in your server startup options or environment — consult the CLI reference for your version's flags.
...
**Apollo Sandbox:**
- **Repository**: Apollo GraphQL organisation on GitHub (open source, Apache 2.0) — [product docs](https://www.apollographql.com/docs/graphos/platform/sandbox)
- **License**: Apache 2.0
...
FraiseQL supports both Apollo Sandbox and GraphiQL. Consult the [CLI reference](/reference/cli) for the startup flags to select your preferred playground tool.
...
| **Is it private?** | Yes, Apollo Sandbox telemetry is opt-in only |
...
3. **Check Apollo's docs** - [Apollo Server documentation](https://www.apollographql.com/docs/apollo-server)
