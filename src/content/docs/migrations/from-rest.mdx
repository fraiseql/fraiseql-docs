...
<Aside type="note">
With FraiseQL, you don't have to abandon REST. FraiseQL serves REST and GraphQL (and gRPC) from the same compiled schema. You can keep your existing REST clients and add GraphQL for new consumers.
</Aside>
...
## Keeping Your REST Clients

Map your existing REST endpoints to `rest_path` annotations — your REST clients continue working unchanged. Add GraphQL for frontends or internal services that benefit from field selection. Introduce gRPC for service-to-service calls when you need binary efficiency.

```python
# Existing GET /users → /rest/v1/users  (default path; configure via [fraiseql.rest] path)
@fraiseql.query(
    rest_path="/users",
    rest_method="GET",
)
def users(limit: int = 100) -> list[User]:
    return fraiseql.config(sql_source="v_user")
# REST clients call GET /rest/v1/users (unchanged behavior)
# GraphQL clients send query { users { ... } } (new option)
```
