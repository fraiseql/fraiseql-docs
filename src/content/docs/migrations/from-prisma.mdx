...
<Tabs>
  ...
  <TabItem label="TypeScript">
    ...
    middleware(async (request, next) => {
        if (shouldUseFraiseQL(request.userId)) {
            return fraiseqlHandler(request);
        }
        return prismaHandler(request);
    });
    ```
  </TabItem>
  <TabItem label="Go">
    ...
    fraiseql.Use(func(req fraiseql.Request, next fraiseql.Handler) fraiseql.Response {
        if shouldUseFraiseQL(req.UserID) {
            return fraiseqlHandler(req)
        }
        return prismaHandler(req)
    })
    ```
  </TabItem>
</Tabs>
