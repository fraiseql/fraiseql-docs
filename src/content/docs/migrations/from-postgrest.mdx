...
4. **Create `fraiseql.toml`**
   ...
   <Aside type="note">
   The `[fraiseql.rest]` configuration section is available since rc.15. See [TOML Configuration Reference](/reference/toml-config/#fraiseqlrest) for all fields.
   </Aside>
...
PostgREST serves at `/` by default (or a configured prefix). FraiseQL serves REST at `/rest/v1` by default. Update your clients accordingly, or configure a custom path via `[fraiseql.rest]` `path` in `fraiseql.toml` (available since rc.15).
