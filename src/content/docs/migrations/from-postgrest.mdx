...
| `GET /users?select=*,orders(*)` | Use a SQL view that joins users and orders (embedding planned for Phase 2) |
...
**Embedding is not yet available.**
PostgREST's `?select=*,orders(*)` syntax for automatic relationship embedding is planned for FraiseQL Phase 2. In the meantime, create SQL views that join the tables you need.
