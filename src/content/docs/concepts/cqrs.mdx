...
This separation extends naturally to FraiseQL's multi-transport support. The `v_*` views produce JSON-shaped responses consumed by GraphQL and REST. For gRPC transport, FraiseQL reads row-shaped views (`rv_*`) that return typed columns rather than a JSONB blob — but they draw from the same underlying `tb_*` write tables. One set of normalized tables, two view shapes, three transport protocols.
  ...
  <TabItem label="PostgreSQL">
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM tb_user WHERE email = user_email) THEN
            RETURN ROW('failed:conflict', 'User with email ' || user_email || ' already exists',
...
<Tabs syncKey="db">
  ...
  <TabItem label="SQL Server">
    ...
    -- With filtering
    SELECT TOP 10 data FROM dbo.v_user
...
2. **Create a write function**:
   ...
   BEGIN
       ...
       RETURN ROW('success', 'Item created', v_item_id::text, 'TestItem',
           (SELECT data FROM v_test_item WHERE id = v_item_id),
           NULL, NULL, NULL)::mutation_response;
   END;
   $$;
   ```
