...
The same principle applies across all transports. A `v_post` view backing a GraphQL query also backs the equivalent REST endpoint. For gRPC, a companion `rv_post` view returns typed columns rather than JSONB, compiled from the same base tables. You write the SQL for both shapes; FraiseQL compiles each to the correct wire format at compile time.
...
<Aside type="tip" title="You review SQL, not black-box output">
  AI generates the view scaffold. You read the SQL in a PR, verify correctness, and commit. The LLM touches nothing in your database — it produces text you review and apply.
</Aside>
...
<LinkCard
  ...
  description="Copy-paste prompts for every scenario: single tables, one-to-many, many-to-many, aggregations, filtered views, and mutation functions. Works with Claude, GPT-4o, and local Ollama models."
