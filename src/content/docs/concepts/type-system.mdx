...
<Aside type="note">
Prefer `@fraiseql.computed` for computed fields added at the schema level — it expresses intent more clearly and works with IDE autocompletion. Use `fraiseql.field(computed=True)` only when defining computed fields inline inside a `@fraiseql.type` class definition using `Annotated`.
</Aside>
