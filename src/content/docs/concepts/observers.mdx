...
<Aside type="note">
Observers run **asynchronously by default** — the mutation returns to the client immediately and observer actions fire in the background (fire-and-forget). Because observers are TOML-configured (not SDK-defined), synchronous observer behavior is not available as a decorator parameter. Use synchronous observers sparingly; they add latency to every matching mutation.
</Aside>
...
The email action sends messages via SMTP using the [lettre](https://github.com/lettre/lettre) crate. It supports TLS, SMTP authentication, and configurable sender/recipient addresses.

**Supported configuration:**
- `from` — Sender email address
- `to` — Recipient email address(es)
- `subject` — Subject line with `{field_name}` substitution
- `body_template` — Body text with `{field_name}` substitution
- `smtp_host` or `smtp_host_env` — SMTP server hostname
- `smtp_port` — SMTP port (default: 587)
- `smtp_user_env` — SMTP username from environment variable
- `smtp_password_env` — SMTP password from environment variable
- `tls` — `"required"` (default), `"opportunistic"`, or `"none"`
...
Old rows accumulate over time and must be pruned by a cleanup job. See the [Observer Operations Runbook](/operations/observer-runbook#idempotency-table-operations) for cleanup strategies and retention policies.
...
<Aside type="note">
The email action uses SMTP via the `lettre` crate. For high-volume transactional email, consider routing through a webhook action to a dedicated email provider API (SendGrid, Resend, Postmark) instead of direct SMTP.
</Aside>
