...
REST endpoints are only created for operations that include `rest_path`/`rest_method` annotations. gRPC methods are only created for operations that include `grpc_service`/`grpc_method` annotations. Operations without transport annotations remain GraphQL-only.
</Aside>
