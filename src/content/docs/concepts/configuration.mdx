...
<Aside type="note">
**Authentication** (`JWT_SECRET`, `JWT_ALGORITHM`, OAuth credentials) is configured via environment variables, not `fraiseql.toml`. **Logging** uses `RUST_LOG`. **Tracing** can be configured via the `[tracing]` TOML section (since v2.1.0) or `OTEL_*` env vars. There are no `[auth]`, `[logging]`, `[graphql]`, or `[metrics]` sections.
...
Multi-transport configuration sections:
...
| `OTEL_EXPORTER_OTLP_ENDPOINT` | OpenTelemetry collector endpoint (fallback; prefer `[tracing] otlp_endpoint` in TOML) |
| `OTEL_SERVICE_NAME` | Service name for traces (fallback; prefer `[tracing] service_name` in TOML) |
...
Enable REST and gRPC alongside GraphQL. All three transports are served from the same port (default: 8080).
...
REST endpoints are only created for operations that include `rest_path`/`rest_method` annotations. gRPC endpoints are auto-generated for all operations when `[grpc]` is enabled — see [gRPC Transport](/features/grpc-transport). Operations without REST annotations remain GraphQL-only (plus gRPC if enabled).
</Aside>
