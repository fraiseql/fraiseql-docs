...
LEFT JOIN v_comment vc ON vc.id = c.id
GROUP BY p.id, p.title, vu.data;
```
...
Author your API once. Serve it to GraphQL clients, REST clients, and gRPC clients simultaneously — no separate services, no gateway, no protocol translation layer.

```
One compiled schema → one Rust binary → three transports on one port
```

Traditional stacks require separate services for different API styles. A REST API, a GraphQL API, and a gRPC service are three codebases, three deployments, three maintenance burdens, three auth configurations. FraiseQL collapses this into one.
...
<CardGrid>
  ...
  <Card title="Performance Benchmarks" icon="chart">
    [Measured performance comparisons](/operations/performance-benchmarks)
  </Card>
</CardGrid>
