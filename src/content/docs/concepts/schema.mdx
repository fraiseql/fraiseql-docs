...
FraiseQL compiles the `sql_source` view name into optimized SQL at build time. At runtime it executes `SELECT data FROM v_user WHERE ...` and maps the result to the return type. No resolver body is needed — `pass` is the correct body for compiled queries.
