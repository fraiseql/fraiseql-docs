...
**Row-shaped views** (gRPC): For operations annotated with `grpc_service`/`grpc_method`, the compiler generates standard SELECT views with typed columns. The server maps columns to protobuf fields — no JSON intermediate. The database does less work, the server does less work, the wire payload is smaller.
