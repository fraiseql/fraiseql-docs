...
import { CardGrid, Card, Steps, Tabs, TabItem, Aside } from '@astrojs/starlight/components';
...
FraiseQL:
- One schema definition (Python, TypeScript, Go, or 6 other languages)
- One `fraiseql compile` step
- One Rust binary serving GraphQL + REST + gRPC

The Python API is the primary schema authoring layer. For teams that prefer a TOML-first workflow, [SpecQL](https://github.com/fraiseql/specql) is an optional companion tool that compiles TOML schema definitions to FraiseQL Python decorators, with 56 built-in semantic scalar types (Email, IBAN, Money, and more).
...
<CardGrid>
  ...
  <Card title="Any Language" icon="pencil">
    Define schemas in Python, TypeScript, Go, Java, Rust, PHP, C#, Elixir, and F#. All compile to the same output.
  </Card>
...
<CardGrid>
  <Card title="Manual Setup" icon="rocket">
    [Install locally and connect your database](/getting-started/quickstart)
  </Card>
