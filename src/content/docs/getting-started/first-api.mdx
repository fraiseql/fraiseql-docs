...
Adding these annotations exposes the operation as `GET /rest/posts`. The default prefix is `/rest`. Configure with `[fraiseql.rest]` `prefix` to use `/api` or any other value. The GraphQL endpoint continues to work unchanged.

```bash
curl http://localhost:8080/rest/posts?limit=5
```
