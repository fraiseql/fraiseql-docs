...
Your API is now also available via REST and gRPC alongside GraphQL. REST endpoints are served at `/rest/` by default when you add `rest_path`/`rest_method` annotations to your schema operations. The default prefix is `/rest`. Configure with `[fraiseql.rest]` `prefix` to use `/api` or any other value. gRPC is served on the same port via HTTP/2 content-type negotiation.
...
curl http://localhost:8080/rest/posts
```
