---
title: Manual Setup
description: Install FraiseQL locally and connect to an existing database
---

import { Tabs, TabItem, Aside } from '@astrojs/starlight/components';
import EmbeddedSandbox from '../../../components/EmbeddedSandbox.astro';

<Aside type="tip">
**Just want to see it work?** The [5-Minute Quickstart](/getting-started/five-minute-quickstart) gets you from zero to a working API with one `docker compose up` command — no installation required.
</Aside>

Get a FraiseQL GraphQL API running with your own database. This guide assumes you have a PostgreSQL database with tables already set up. For the full workflow including database setup with 🍯 Confiture, see [Your First API](/getting-started/first-api).

## Prerequisites

- PostgreSQL database with tables created
- Your preferred schema language installed (Python 3.10+, Node.js 18+, or Go 1.21+)

## Step 1: Install FraiseQL

FraiseQL is a Rust binary. Install it once, use it from any project regardless of language.

```bash
# Install script (macOS / Linux)
curl -fsSL https://fraiseql.dev/install.sh | sh

# From source (requires Rust toolchain)
cargo install fraiseql
```

{/* source: releasing.md:L625-L645 at frozen SHA d0a4ed4 — Homebrew formula publishing is a Phase 4 "Later" item; no Homebrew distribution exists yet. */}

Verify:

```bash
fraiseql --version
```

## Step 1b: Install the Schema SDK

The schema SDK lets you define GraphQL types in your preferred language using decorators or structs.

<Tabs syncKey="lang">
  <TabItem label="Python">
    ```bash
    uv add fraiseql
    # or: pip install fraiseql
    ```
  </TabItem>
  <TabItem label="TypeScript">
    ```bash
    npm install fraiseql
    # or: pnpm add fraiseql
    ```
  </TabItem>
  <TabItem label="Go">
    ```bash
    go get github.com/fraiseql/fraiseql-go
    ```
  </TabItem>
</Tabs>

## Step 2: Write Your SQL Views

<Aside type="tip">
The `tb_`/`v_`/`fn_` prefixes aren't mandatory, but FraiseQL uses them to find your objects automatically. `fraiseql compile` scans for views matching `v_{query_name}` and functions matching `fn_{mutation_name}`. If you already have a different naming convention, you can override it with `sql_source="my_posts_view"` on the decorator — but following the convention means less config everywhere.
</Aside>

Create SQL views that follow the `.data` JSONB pattern. Each view has an `id` column and a `data` column:

<Tabs syncKey="db">
  <TabItem label="PostgreSQL">
    ```sql title="db/schema/02_read/v_user.sql"
    CREATE VIEW v_user AS
    SELECT
        u.id,
        jsonb_build_object(
            'id', u.id::text,
            'name', u.name,
            'email', u.email,
            'created_at', u.created_at
        ) AS data
    FROM tb_user u;
    ```

    ```sql title="db/schema/02_read/v_post.sql"
    CREATE VIEW v_post AS
    SELECT
        p.id,
        jsonb_build_object(
            'id', p.id::text,
            'title', p.title,
            'content', p.content,
            'author', vu.data
        ) AS data
    FROM tb_post p
    JOIN tb_user u ON u.pk_user = p.fk_user
    JOIN v_user vu ON vu.id = u.id;
    ```
  </TabItem>

  <TabItem label="MySQL">
    ```sql title="db/schema/02_read/v_user.sql"
    CREATE VIEW v_user AS
    SELECT
        u.id,
        JSON_OBJECT(
            'id', u.id,
            'name', u.name,
            'email', u.email,
            'created_at', u.created_at
        ) AS data
    FROM tb_user u;
    ```

    ```sql title="db/schema/02_read/v_post.sql"
    CREATE VIEW v_post AS
    SELECT
        p.id,
        JSON_OBJECT(
            'id', p.id,
            'title', p.title,
            'content', p.content,
            'author', vu.data
        ) AS data
    FROM tb_post p
    JOIN tb_user u ON u.pk_user = p.fk_user
    JOIN v_user vu ON vu.id = u.id;
    ```
  </TabItem>

  <TabItem label="SQLite">
    ```sql title="db/schema/02_read/v_user.sql"
    CREATE VIEW v_user AS
    SELECT
        u.id,
        json_object(
            'id', u.id,
            'name', u.name,
            'email', u.email,
            'created_at', u.created_at
        ) AS data
    FROM tb_user u;
    ```

    {/* source: scripts/docs-test/fixtures/sqlite/_smoke.sql:L51-L69 — DEVIATION: SQLite returns json_object output as TEXT; the outer json_object embeds vu.data as an escaped JSON string unless wrapped in json(...) to re-parse it as a nested object. */}
    ```sql title="db/schema/02_read/v_post.sql"
    CREATE VIEW v_post AS
    SELECT
        p.id,
        json_object(
            'id', p.id,
            'title', p.title,
            'content', p.content,
            'author', json(vu.data)
        ) AS data
    FROM tb_post p
    JOIN tb_user u ON u.pk_user = p.fk_user
    JOIN v_user vu ON vu.id = u.id;
    ```
  </TabItem>

  <TabItem label="SQL Server">
    {/* source: scripts/docs-test/fixtures/mssql/_smoke.sql:L18-L23 — DEVIATION: SCHEMABINDING requires referenced views to also be schema-bound; v_post references v_user, so SCHEMABINDING is dropped here. */}
    ```sql title="db/schema/02_read/v_user.sql"
    CREATE VIEW dbo.v_user AS
    SELECT
        u.id,
        (
            SELECT u.id, u.name, u.email, u.created_at
            FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
        ) AS data
    FROM dbo.tb_user u;
    ```

    {/* source: scripts/docs-test/fixtures/mssql/_smoke.sql:L73-L89 — DEVIATION 1: SCHEMABINDING dropped (view-on-view); DEVIATION 2: vu.data is NVARCHAR(MAX) holding JSON text — FOR JSON serialises it as a string literal unless wrapped in JSON_QUERY() to mark the column as already-JSON. */}
    ```sql title="db/schema/02_read/v_post.sql"
    CREATE VIEW dbo.v_post AS
    SELECT
        p.id,
        (
            SELECT p.id, p.title, p.content, JSON_QUERY(vu.data) AS author
            FOR JSON PATH, WITHOUT_ARRAY_WRAPPER
        ) AS data
    FROM dbo.tb_post p
    JOIN dbo.tb_user u ON u.pk_user = p.fk_user
    JOIN dbo.v_user vu ON vu.id = u.id;
    ```
  </TabItem>
</Tabs>

Apply these views to your database (or use `confiture build` — see [🍯 Confiture](/confiture)).

<Aside type="caution" title="Server adapter coverage">
The per-DB view SQL above is verified against each engine. At the time of writing, the `fraiseql-server` binary itself only routes PostgreSQL end-to-end — the MySQL, SQLite, and SQL Server adapters exist in `fraiseql-db` but are not wired into the server's adapter factory. Track the regression signal that unblocks the binary-driven multi-DB happy path in [fraiseql#327](https://github.com/fraiseql/fraiseql/issues/327).
</Aside>

{/* source: scripts/docs-test/fixtures/postgres/_smoke.sql + fixtures/{mysql,sqlite,mssql}/_smoke.sql — per-DB view SQL verified against each engine via pages/_smoke.docs-test.sh; server binary adapter limitation tracked in fraiseql#327 (FW-2). */}

## Step 3: Define Your GraphQL Schema

<Tabs syncKey="lang">
  <TabItem label="Python">
    Create a file called `schema.py`:

    ```python title="schema.py"
    import fraiseql
    from fraiseql.scalars import ID, Email, DateTime

    @fraiseql.type
    class User:
        """A user in the system."""
        id: ID
        name: str
        email: Email
        created_at: DateTime

    @fraiseql.type
    class Post:
        """A blog post."""
        id: ID
        title: str
        content: str
        author: User
        published: bool
    ```
  </TabItem>
  <TabItem label="TypeScript">
    Create a file called `schema.ts`:

    ```typescript title="schema.ts"
    import { type } from 'fraiseql';
    import { ID, Email, DateTime } from 'fraiseql/scalars';

    @type()
    class User {
      /** A user in the system. */
      id: ID;
      name: string;
      email: Email;
      createdAt: DateTime;
    }

    @type()
    class Post {
      /** A blog post. */
      id: ID;
      title: string;
      content: string;
      author: User;
      published: boolean;
    }
    ```
  </TabItem>
  <TabItem label="Go">
    Create a file called `schema.go`:

    ```go title="schema.go"
    package schema

    import "github.com/fraiseql/fraiseql-go/scalars"

    // User represents a user in the system.
    type User struct {
        ID        scalars.ID       `fraiseql:"type"`
        Name      string
        Email     scalars.Email
        CreatedAt scalars.DateTime
    }

    // Post represents a blog post.
    type Post struct {
        ID        scalars.ID `fraiseql:"type"`
        Title     string
        Content   string
        Author    User
        Published bool
    }
    ```
  </TabItem>
</Tabs>

## Step 4: Configure Your Project

Create `fraiseql.toml` with your database connection string:

<Tabs syncKey="db">
  <TabItem label="PostgreSQL">
    ```toml title="fraiseql.toml"
    [project]
    name = "quickstart"
    version = "0.1.0"
    database_target = "postgresql"

    [database]
    url = "postgresql://localhost:5432/mydb"

    [server]
    port = 8080
    host = "127.0.0.1"
    ```
  </TabItem>

  <TabItem label="MySQL">
    ```toml title="fraiseql.toml"
    [project]
    name = "quickstart"
    version = "0.1.0"
    database_target = "mysql"

    [database]
    url = "mysql://localhost:3306/mydb"

    [server]
    port = 8080
    host = "127.0.0.1"
    ```
  </TabItem>

  <TabItem label="SQLite">
    ```toml title="fraiseql.toml"
    [project]
    name = "quickstart"
    version = "0.1.0"
    database_target = "sqlite"

    [database]
    url = "sqlite://./mydb.db"

    [server]
    port = 8080
    host = "127.0.0.1"
    ```
  </TabItem>

  <TabItem label="SQL Server">
    ```toml title="fraiseql.toml"
    [project]
    name = "quickstart"
    version = "0.1.0"
    database_target = "sqlserver"

    [database]
    url = "sqlserver://localhost:1433;Database=mydb;Trusted_Connection=true"

    [server]
    port = 8080
    host = "127.0.0.1"
    ```
  </TabItem>
</Tabs>

## Step 5: Compile and Serve

```bash
# Compile the mapping between your types and SQL views
fraiseql compile

# Start the server
fraiseql run
```

You should see:

```
✓ Compiled 2 types → mapped to SQL views
✓ Built query executor
→ GraphQL API running at http://localhost:8080/graphql
```

<Aside type="tip">
If your schema uses `inject_params` (e.g., multi-tenant schemas), add `--dev-claims '{"tenant_id": "dev"}'` to bypass JWT requirements during local development. See [Dev Mode](/guides/dev-mode).
</Aside>

Your API is now also available via REST and gRPC alongside GraphQL. REST endpoints are served at `/rest/v1/` by default when you add `rest_path`/`rest_method` annotations to your schema operations. The default path is `/rest/v1`. Configure with `[rest]` `path` to use `/api` or any other value. gRPC is served on the same port via HTTP/2 content-type negotiation.

To expose an operation via REST, add transport annotations to your query decorator:

```python
@fraiseql.query(
    sql_source="v_post",
    rest_path="/posts",
    rest_method="GET",
)
def posts(limit: int = 10) -> list[Post]: ...
```

With that annotation in place, both of the following work simultaneously:

```bash
# GraphQL (always available)
curl -X POST http://localhost:8080/graphql \
  -H "Content-Type: application/json" \
  -d '{"query": "{ posts { id title } }"}'

# REST (available when rest_path annotation is present)
curl http://localhost:8080/rest/v1/posts
```

## Step 6: Query Your API

Open your browser to `http://localhost:8080/graphql` or use curl:

```bash
curl -s -X POST http://localhost:8080/graphql \
  -H "Content-Type: application/json" \
  -d '{"query": "{ users { id name email } }"}' | jq .
```

Response:

```json
{
  "data": {
    "users": [
      {
        "id": "1",
        "name": "Alice",
        "email": "alice@example.com"
      }
    ]
  }
}
```

If a query references a field that doesn't exist, FraiseQL returns a structured error:

```json
{
  "errors": [
    {
      "message": "Cannot query field 'phone' on type 'User'",
      "locations": [{ "line": 1, "column": 28 }]
    }
  ]
}
```

## What Just Happened?

1. **SQL views created** — You wrote views following the `.data` JSONB pattern
2. **Schema compiled** — FraiseQL mapped your GraphQL types to those SQL views
3. **Query executor built** — Pre-compiled query paths for each type
4. **API served** — Every GraphQL query resolves to a single SQL statement

No resolvers. No N+1 problems. No generated SQL you can't read.

## Try It Without Installing

Query the same schema — users, posts, comments, tags — against a live FraiseQL server backed by real PostgreSQL. No account required.

<EmbeddedSandbox
  endpoint="https://demo.fraiseql.dev/graphql"
  title="FraiseQL Demo API"
  description="Try the schema from this guide against a live server."
  height="520px"
  preloadedQuery={`query {
  posts(limit: 3) {
    id
    title
    author {
      name
    }
    tags {
      name
    }
  }
}`}
/>

<Aside type="tip" title="Sanity check">
If anything doesn't work, run `fraiseql doctor` to check your schema, config, database connection, and secrets in one step. See the [CLI reference](/reference/cli#fraiseql-doctor) for details.
</Aside>

:::note[Before going to production]
The `fraiseql.toml` in this guide is a minimal development configuration. Before deploying:

- Enable TLS (`[server.tls]`) or terminate at your load balancer
- Configure authentication: set `JWT_SECRET` environment variable
- Set `admin_token` (or `FRAISEQL_ADMIN_TOKEN`) to enable and protect the RBAC management API
- Review the [production checklist](/guides/deployment#production-checklist)

`ServerConfig::default()` (used in unit tests and local dev) disables TLS, authentication,
and rate limiting. Always use `ServerConfig::from_file("fraiseql.toml")` in production.
:::

## Next Steps

- [Your First API](/getting-started/first-api) — Build a complete blog API with 🍯 Confiture
- [Developer-Owned SQL](/concepts/developer-owned-sql) — Why you write the views
- [TOML Configuration](/concepts/configuration) — All configuration options
- [Schema Definition](/concepts/schema) — Advanced schema patterns