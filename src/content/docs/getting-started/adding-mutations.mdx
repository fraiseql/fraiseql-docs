...
<Tabs syncKey="db">
  ...
  <TabItem label="MySQL">
    <Aside type="caution">
    **MySQL mutations are not supported in the current release.** The MySQL adapter's `execute_function_call` returns `Unsupported` — mutation functions are not executed. MySQL stored functions also cannot return the eight-column `mutation_response` composite type that the FraiseQL runtime expects (MySQL has no equivalent of PostgreSQL composite types).

    Use MySQL for read queries only. Mutation support is planned for a future release. For write operations on MySQL, consider running a PostgreSQL instance alongside MySQL, or use PostgreSQL as your primary database.
    </Aside>
  </TabItem>
