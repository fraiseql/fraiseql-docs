...
MySQL mutations use stored procedures with a JSON output convention instead of PostgreSQL composite types. Define your mutation function as a stored procedure that returns a single JSON column:
...
BEGIN
  ...
  INSERT INTO post (id, title, body, created_at)
  VALUES (v_id, p_title, p_body, NOW());
  SELECT JSON_OBJECT(
    'id', v_id,
    'title', p_title,
    'body', p_body
...
The Python mutation declaration is the same as PostgreSQL:

```python
@fraiseql.mutation(sql_source="fn_create_post", operation="CUSTOM")
def create_post(title: str, body: str) -> Post: pass
```
...
<Tabs syncKey="lang">
  ...
  <TabItem label="TypeScript">
    ...
    import { mutation, input } from 'fraiseql';
    ...
    @mutation()
    function publishPost(input: PublishPostInput): Post {
      /** Publish a post. */
    }
    ```
  </TabItem>
...
With that annotation, the mutation is reachable via REST alongside GraphQL. The default REST prefix is `/rest`. Configure with `[fraiseql.rest]` `prefix` to use `/api` or any other value:

```bash
curl -X POST http://localhost:8080/rest/posts \
