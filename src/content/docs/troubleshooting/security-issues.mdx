...
3. For FraiseQL, set tenant context in middleware before queries:

```python
@fraiseql.middleware
async def set_user_context(request, next):
...
**Solutions**:
```bash
# 1. Filter sensitive fields from logs
@fraiseql.middleware
async def sanitize_logs(request, response):
    # Remove password, token from logs
    if request.body:
        body = request.body
        body['password'] = '***REDACTED***'
    log_request(body)  # Safe to log
...
# 3. In production, use generic errors
@fraiseql.error_handler
def handle_not_found(error):
    # Don't include what wasn't found
