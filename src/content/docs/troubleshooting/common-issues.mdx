...
**Problem**: `GET /rest/users` returns 404 (the default prefix is `/rest`; configure with `[fraiseql.rest]` `prefix`)

**Solutions**:
```bash
# 1. Check resource name matches schema query/mutation name
# Schema: def users(...) -> list[User]
# REST path: /rest/users (matches)
...
curl -X POST http://localhost:8080/rest/createUser \
