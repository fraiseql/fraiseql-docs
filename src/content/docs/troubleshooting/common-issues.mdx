...
**Problem**: `GET /rest/v1/users` returns 404 (the default path is `/rest/v1`; configure with `[fraiseql.rest]` `path`)
...
# 3. Confirm REST is enabled (it is by default in v2.0.1+)
