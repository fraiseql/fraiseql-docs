...
# But verify in logs: RUST_LOG=debug
...
# 4. Define role requirement on mutation
@fraiseql.mutation(requires_role="editor")
...
# 5. Cache expensive computations using cache_ttl_seconds in fraiseql.config()
@fraiseql.query
def expensive_query() -> list[Result]:
    return fraiseql.config(sql_source="v_expensive", cache_ttl_seconds=3600)
```
...
**Solutions**:
```python
# 1. Increase TTL using cache_ttl_seconds in fraiseql.config()
@fraiseql.query
def my_cached_query() -> list[Result]:
    return fraiseql.config(sql_source="v_my_query", cache_ttl_seconds=86400)  # 24 hours instead of 1 hour
...
**Solutions**:
```python
# 1. Define and register the custom scalar using @fraiseql.scalar.
# The decorator registers it globally — no separate register call is needed.
import fraiseql
from fraiseql import CustomScalar
...
# 2. Use in schema — the scalar is already registered by the decorator above
@fraiseql.type
class Post:
    metadata: JSON  # Now valid — no further registration required
```
...
## REST Transport Issues

### REST Endpoint Returns 404
**Problem**: `GET /rest/v1/users` returns 404 (the default path is `/rest/v1`; configure with `[rest]` `path`)

{/* source: CHANGELOG.md v2.3.0 entry — REST transport landed in the v2.1 line and is on by default in v2.3.x. The v2.0.1 anchor predated the v2.1.0 release baseline. */}
**Solutions**:
```bash
# 1. Check resource name matches schema query/mutation name
# Schema: def users(...) -> list[User]
# REST path: /rest/v1/users (matches)

# 2. Verify fraiseql started without errors
fraiseql run  # Check startup logs

# 3. Confirm REST is enabled (it is by default in v2.3.0+)

# 4. Test GraphQL endpoint to confirm API is running
curl http://localhost:8080/graphql -d '{"query":"{ __typename }"}'
```

### REST Request Body Not Accepted
**Problem**: POST to REST mutation fails with 400 or 415

**Solutions**:
```bash
# Send Content-Type: application/json header
curl -X POST http://localhost:8080/rest/v1/createUser \
...
## gRPC Transport Issues

### gRPC Connection Fails (UNAVAILABLE)
**Problem**: gRPC client gets `Status.UNAVAILABLE`

**Causes**:
- Proxy or load balancer does not support HTTP/2
- TLS mismatch between client and server
- FraiseQL not started

**Solutions**:
```bash
# 1. Check FraiseQL is running
curl http://localhost:8080/health

# 2. Verify with grpcurl (no TLS, local)
grpcurl -plaintext localhost:50052 list

# 3. Check load balancer HTTP/2 configuration
# nginx: use grpc_pass not proxy_pass
# AWS ALB: set protocol version to GRPC
# Kubernetes nginx-ingress: set backend-protocol: "GRPC"
```

### gRPC Returns Wrong HTTP Version
**Problem**: gRPC client logs "received trailing metadata size exceeds limit" or "RST_STREAM"

**Cause**: Proxy is speaking HTTP/1.1 to FraiseQL instead of HTTP/2. gRPC is incompatible with HTTP/1.1.

**Solution**: Configure the upstream proxy for HTTP/2 backend connections. See the [Kubernetes](/deployment/kubernetes#grpc-and-http2) and [Docker](/deployment/docker#multi-transport-in-docker) deployment guides.
