...
- Check the URL path — default is `/rest/v1/`; configurable via `[fraiseql.rest]` `path`
...
- Verify `require_auth` in `[fraiseql.rest]` is set to `true` when you expect auth enforcement.
