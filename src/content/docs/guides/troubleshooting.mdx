...
If FraiseQL isn't working, start with `fraiseql doctor` — it runs 11 checks covering schema, config, database, secrets, and feature coherence in a single command:

```bash
fraiseql doctor
```

If `doctor` passes but the issue persists, continue with the manual checklist below. To update the schema without restarting, use `kill -USR1` or the [admin reload endpoint](/reference/admin-api#schema-reload).
...
### Request Rejected: Too Many Variables
...
- Check the URL prefix — default is `/rest/`; configurable via `[fraiseql.rest]` `prefix` or `FRAISEQL_REST_PREFIX`
- Confirm `[fraiseql.rest]` is enabled in `fraiseql.toml` (rc.15+)

**REST response is wrapped in `{"data": ...}`**
- You may be hitting the GraphQL endpoint (`/graphql`) rather than the REST endpoint (`/rest/...`). FraiseQL's REST transport returns unwrapped JSON directly.
...
- Verify `auth` in `[fraiseql.rest]` is not set to `"none"` when you expect auth enforcement.
...
- `grpc_service` and `grpc_method` values are case-sensitive. Verify they match the service and method name exactly as declared in your SDK annotations.
- Use `grpcurl -plaintext localhost:8080 list` to see available services.

---

See [Observability](/features/observability) for Prometheus metrics, OpenTelemetry tracing, and structured logging — these are the primary tools for diagnosing production issues.
