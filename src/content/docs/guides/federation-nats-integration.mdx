...
Always write with federation first, then publish events — never publish before the federation operation is complete.
</Aside>

<Aside type="note">
**2. Idempotent Publishing**

Include an entity ID in the event and use an idempotency key. If publish fails, retrying is safe.
</Aside>

<Aside type="caution">
**3. Compensation Across Both Systems**

When compensating a saga, undo both federation changes AND publish notifications about the cleanup.
</Aside>
...
<CardGrid>
  <Card title="Federation Configuration" icon="setting">
    [Multi-Database Setup](/guides/federation-configuration) — Synchronous federation reference
  </Card>
  <Card title="Advanced NATS" icon="external">
    [Event-Driven Patterns](/guides/advanced-nats) — Async messaging reference
  </Card>
  <Card title="Advanced Federation Patterns" icon="puzzle">
    [Enterprise Patterns](/guides/advanced-federation) — Saga orchestration
  </Card>
</CardGrid>
