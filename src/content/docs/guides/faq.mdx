...
No. Add `rest_path`/`rest_method` annotations to expose operations as REST, `grpc_service`/`grpc_method` for gRPC, or both. Operations without annotations are GraphQL-only. Each operation can be on any combination of transports.
