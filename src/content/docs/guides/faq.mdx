...
A: FraiseQL supports per-endpoint rate limits configured in `fraiseql.toml` under `[fraiseql.security.rate_limiting]`. Override at runtime with environment variables such as `RATE_LIMIT_AUTH_START`.
