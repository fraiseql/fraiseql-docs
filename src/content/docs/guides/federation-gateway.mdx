...
description: Run fraiseql gateway to federate multiple FraiseQL subgraphs without Apollo Router
---

import { Aside, Tabs, TabItem } from '@astrojs/starlight/components';

:::caution[Beta feature]
The built-in federation gateway is available as a beta feature. Core routing and entity resolution work today. Advanced features (subscription forwarding, `@requires` pre-fetching) are planned for future releases.
:::

## When to Use the Built-in Gateway

The `fraiseql gateway` command federates multiple FraiseQL subgraphs into a single endpoint without Apollo Router.
...
Client
  │
  ▼
fraiseql gateway :4000     ← type-level routing, entity resolution
  │         │
  ▼         ▼
User        Order
Service     Service
...
- Replace `rover supergraph compose` + `router` commands with `fraiseql gateway`
