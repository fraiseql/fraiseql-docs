...
Configure your OIDC provider in `fraiseql.toml`. The Rust runtime uses these settings to validate JWTs on every request.
...
The `client_secret` is **never** placed in `fraiseql.toml`. Supply it at runtime via the `OIDC_CLIENT_SECRET` environment variable.
...
Security behaviour is configured in `fraiseql.toml` under `[fraiseql.security.*]`. These settings are baked into the compiled schema and enforced by the Rust runtime.
...
Configure the `[auth]` section to point at the proxy's OIDC endpoint. The proxy handles SAML assertion validation and attribute mapping — no changes to FraiseQL are needed.
