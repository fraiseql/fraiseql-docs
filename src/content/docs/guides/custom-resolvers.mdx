...
For Python-level errors from middleware:

```python
@fraiseql.middleware
def check_rate_limit(request, next):
    if is_rate_limited(request.client_ip):
        from fraiseql.errors import GraphQLError
        raise GraphQLError(
            "Rate limit exceeded",
            extensions={"code": "RATE_LIMITED", "retry_after": 60},
        )
    return next(request)
```
