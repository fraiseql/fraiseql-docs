...
<Aside type="caution">
The `[caching]` and `[[caching.rules]]` TOML namespaces shown below are not verified as valid `fraiseql.toml` sections. See the [Caching feature page](/features/caching) for current configuration details and status.
</Aside>
...
<Aside type="caution">
The `[graphql.apq]` TOML namespace is not verified as a valid `fraiseql.toml` section. See the [APQ feature page](/features/apq) for current configuration details.
</Aside>
