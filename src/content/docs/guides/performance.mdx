...
<Aside type="caution">
The `[caching]` TOML section is unverified and may cause a hard parse error at startup. Treat examples in this section as aspirational until the section is confirmed stable. See the [Caching](/features/caching) reference for the latest status.
</Aside>

The `[caching]` section is available in `fraiseql.toml`. See [Caching](/features/caching) for full configuration reference.
...
FraiseQL automatically invalidates cache entries when mutations run. Declare which views a mutation invalidates with `invalidates_views=` on the `@fraiseql.mutation` decorator:

```python
@fraiseql.mutation(
    sql_source="fn_create_post",
    operation="CREATE",
    invalidates_views=["v_post"],   # flush these view caches after the mutation
)
def create_post(input: PostInput) -> PostResult:
    pass
```

For UPDATE and DELETE mutations that return an `entity_id`, FraiseQL performs entity-aware eviction — only the cache entries for that specific entity are purged rather than flushing the entire view. TTL is set per-query via `cache_ttl_seconds=` on `@fraiseql.query`. See [Caching](/features/caching) for the full reference.
...
Pagination limits are enforced at the query level via the `limit` argument default and any server-side cap configured through your query validators. There is no `[graphql.pagination]` TOML section.
...
All three transports share the same connection pool and Executor. Transport choice is rarely the performance bottleneck — database query time dominates.

| Transport | View shape | Wire format | Notes |
|-----------|-----------|-------------|-------|
| GraphQL | JSON-shaped (`json_agg`) | JSON text | DB produces JSON; server passes through |
| REST | JSON-shaped (`json_agg`) | JSON text | Same as GraphQL — identical views |
| gRPC | Row-shaped (`SELECT`) | Protobuf binary | DB skips JSON; server maps to protobuf |

The gRPC path does less work at both the DB and server level, but for most workloads the difference is small because the DB query itself (joins, indexes, RLS) dominates latency.

Optimize the DB query first. Transport choice is a secondary concern.
...
<CardGrid>
  ...
  <Card title="Observability" icon="setting">
    Prometheus metrics, OpenTelemetry tracing, and structured logging. [Observability](/features/observability)
  </Card>
