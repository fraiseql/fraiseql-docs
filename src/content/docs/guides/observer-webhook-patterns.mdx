...
When `backend = "nats"`, the Rust runtime publishes a message to a NATS subject after each successful mutation. The subject follows the pattern `fraiseql.{entity_type}.{operation}` — for example, `fraiseql.Order.CREATE`.
...
async def main():
    ...
    await nc.subscribe("fraiseql.Order.CREATE", cb=handle_order_created)
...
<CardGrid>
  ...
  <Card title="Mutations Guide" icon="pencil">
    [Writing Mutations](/guides/mutations) — How to define mutations and SQL functions
  </Card>
</CardGrid>
