...
JWT authentication is configured via environment variables, not `fraiseql.toml` (see [Environment Variables](#environment-variables) below). There is no `[auth]` TOML section.
