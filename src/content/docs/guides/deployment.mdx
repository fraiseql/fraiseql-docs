...
For JWT token validation, configure `JWT_SECRET` and `JWT_ALGORITHM` environment variables (see [Environment Variables](#environment-variables) below). For OIDC/OAuth2 flows, use the `[security.pkce]` TOML section with `client_id`, `issuer_url`, and `redirect_uri`.
...
<Aside type="note">
gRPC requires HTTP/2. Ensure your load balancer or reverse proxy supports HTTP/2 pass-through or termination. In nginx, add `listen 443 ssl http2;` to your server block. Most managed load balancers (AWS ALB, GCP Cloud Load Balancing, Azure Application Gateway) support HTTP/2 by default.
</Aside>
