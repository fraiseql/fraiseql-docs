...
<Aside type="note">
REST transport requires the `rest-transport` Cargo feature and is available in the published Docker image at `ghcr.io/fraiseql/server:latest`. Configuration via `[fraiseql.rest]` in `fraiseql.toml` is available since v2.1.
</Aside>
