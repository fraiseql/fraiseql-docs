...
## Raising Errors from Middleware

Use `GraphQLError` for errors that should stop request processing:
...
@fraiseql.middleware
def check_maintenance_mode(request, next):
    if is_maintenance_mode():
        raise GraphQLError(
            ...
            extensions={
                ...
                "statusCode": 503,
                "retryAfter": get_maintenance_end_timestamp(),
            },
        )
    return next(request)

@fraiseql.middleware
def enforce_rate_limit(request, next):
...
import { middleware, GraphQLError } from 'fraiseql';
  ...
  if (isMaintenanceMode()) {
    throw new GraphQLError('Service temporarily unavailable', {
      extensions: {
        code: 'SERVICE_UNAVAILABLE',
        statusCode: 503,
        retryAfter: getMaintenanceEndTimestamp(),
      },
    });
  }
  return next(request);
