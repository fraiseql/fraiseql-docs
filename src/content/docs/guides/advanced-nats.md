...
subjects: {
  label: "🏷️ Subject Space\n(events.orders.*)\n(events.users.*)\n(commands.*)"
  shape: box
}

clients.publisher -> nats_cluster.server1: "Publish"
clients.subscriber_a -> nats_cluster.server2: "Subscribe"
clients.subscriber_b -> nats_cluster.server3: "Subscribe"

nats_cluster.server1 -> persistence.jetstream: "Persist"
nats_cluster.server1 -> subjects: "Route by subject"
```
...
```python
from fraiseql import nats, circuit_breaker

class PaymentServiceCircuitBreaker:
    def __init__(self):
        self.failure_count = 0
        self.failure_threshold = 5
        self.reset_timeout = 60000  # 60s
    ...
    async def call(self, func, *args):
        ...
        try:
            ...
            if self.failure_count >= self.failure_threshold:
                self.state = "open"
                await asyncio.sleep(self.reset_timeout / 1000)
                self.state = "half-open"
            raise

# Usage
payment_cb = PaymentServiceCircuitBreaker()

@nats.subscribe("events.orders.confirmed")
async def process_order_payment(message):
    """Process with circuit breaker."""
    try:
        await payment_cb.call(charge_customer, message.data)
        await message.ack()
    except Exception as e:
...
payment_service: {
  ...
  listen_reserved: "Listen for reserved"
...
order_service.create_order -> order_service.publish
order_service.publish -> nats

nats -> inventory_service.listen
inventory_service.listen -> inventory_service.reserve
inventory_service.reserve -> nats

nats -> payment_service.listen_reserved
payment_service.listen_reserved -> payment_service.charge
payment_service.charge -> nats

nats -> shipping_service.listen_paid
shipping_service.listen_paid -> shipping_service.create_label
```
...
[nats.jetstream]
storage = "file"
storage_dir = "/data/nats"
max_bytes = 10737418240  # 10GB

[nats.publishing]
auto_publish = true  # Auto-publish mutations
batch_size = 100
batch_timeout = 1000
```
...
- [ ] Regular testing of replay scenarios
- [ ] Capacity planning for stream storage
- [ ] Version your events in headers
