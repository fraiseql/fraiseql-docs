...
<Tabs syncKey="db">
    ...
    BEGIN
        ...
        IF v_fk_user IS NULL THEN
            RETURN ROW('failed:not_found', 'author not found: ' || p_author_id,
                NULL, 'Comment', NULL, NULL, NULL, NULL)::mutation_response;
        END IF;
        ...
        IF p_parent_id IS NOT NULL THEN
            ...
            IF v_fk_parent IS NULL THEN
                RETURN ROW('failed:not_found',
                    'parent comment ' || p_parent_id || ' not found on post ' || p_post_id,
                    NULL, 'Comment', NULL, NULL, NULL, NULL)::mutation_response;
            END IF;
        END IF;
        ...
        RETURN ROW('success', 'Comment created', v_id, 'Comment',
            (SELECT data FROM v_comment WHERE id = v_id),
  ...
  <TabItem label="MySQL">
    MySQL stored functions cannot return composite types. Use a stored procedure that returns a result set shaped as `mutation_response` fields, or return the new entity's UUID and handle errors via SIGNAL.
