...
An operation with no `rest_path` is GraphQL-only. Add `rest_path` and `rest_method` and it's also available as a REST endpoint. Add `grpc_service` and `grpc_method` and gRPC too.
