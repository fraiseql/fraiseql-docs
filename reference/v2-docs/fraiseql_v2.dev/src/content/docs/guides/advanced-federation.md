...
cache_layer: {
  ...
  federation_cache: "Federation Results"
  query_cache: "Query Cache"
}

client -> graphql_server.parser: "GraphQL Query"

graphql_server.parser -> graphql_server.planner
graphql_server.planner -> graphql_server.executor

graphql_server.executor -> cache_layer.federation_cache: "Check cache"
graphql_server.executor -> primary_db.table: "Fetch base entities"
graphql_server.executor -> secondary_db.table: "Fetch federated data (batched)"

primary_db.table -> graphql_server.executor: "User data"
secondary_db.table -> graphql_server.executor: "Analytics data"

graphql_server.executor -> cache_layer.query_cache: "Store results"
cache_layer.query_cache -> client: "Combined response"
```
...
#### Saga Execution Flow (D2 Diagram)

```d2
direction: down

start: {
  label: "Start: Create Order"
  shape: box
}
...
compensate2: {
  label: "↩️ Cancel Order"
  shape: box
}

compensate1: {
  label: "↩️ Release Inventory"
  shape: box
}

rollback: {
  label: "Rollback: All Changes Undone"
  shape: box
}
...
step3_try -> step3_fail: "Payment fails"
step3_fail -> compensate2: "Trigger compensation"
compensate2 -> compensate1
compensate1 -> rollback

step1 -> step1: "mark: {style: {fill: \"#ccffcc\"}}"
step2 -> step2: "mark: {style: {fill: \"#ccffcc\"}}"
step3_fail -> step3_fail: "mark: {style: {fill: \"#ffcccc\"}}"
rollback -> rollback: "mark: {style: {fill: \"#ffcccc\"}}"
```
...
clients: {
  ...
  tenant_a: "Tenant A (Acme Corp)"
  tenant_b: "Tenant B (TechCo)"
  tenant_c: "Tenant C (GlobalInc)"
}
...
tenant_dbs: {
  ...
  tenant_a_db: "PostgreSQL\n(Tenant A Data)"
  tenant_b_db: "PostgreSQL\n(Tenant B Data)"
  tenant_c_db: "PostgreSQL\n(Tenant C Data)"
}
...
clients.tenant_a -> api_gateway.auth
clients.tenant_b -> api_gateway.auth
clients.tenant_c -> api_gateway.auth

api_gateway.router -> tenant_dbs.tenant_a_db: "Tenant A"
api_gateway.router -> tenant_dbs.tenant_b_db: "Tenant B"
api_gateway.router -> tenant_dbs.tenant_c_db: "Tenant C"

tenant_dbs.tenant_a_db -> shared_services.analytics: "Sync metrics"
tenant_dbs.tenant_b_db -> shared_services.analytics
tenant_dbs.tenant_c_db -> shared_services.analytics

shared_services.audit_log <- api_gateway.router: "Log requests"
shared_services.master <- api_gateway.auth: "Validate tenant"
```
...
```python
# Automatic span creation for federation
# Visible in Jaeger, DataDog, New Relic, etc.
