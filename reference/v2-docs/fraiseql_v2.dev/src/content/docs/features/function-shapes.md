...
|-------|------|-------------|
| **updated** | Array | Entities created or updated with `id`, `operation` (CREATED or UPDATED), `entity` (full data), and `__typename` (auto-added by GraphQL) |
| **deleted** | Array | Entities deleted with `id`, `operation` ('DELETED'), `entity` (entity state at deletion), and `__typename` (auto-added by GraphQL) |
...
**Key Point**: SQL functions return `__typename` for each cascade entity. This helps the GraphQL executor resolve the correct type. GraphQL automatically includes this field in responses even though it's not defined in the type schema.
...
```sql
CREATE FUNCTION fn_create_post_with_tags(
    author_id UUID,
    title TEXT,
    content TEXT,
    tag_names TEXT[]
...
BEGIN
    ...
    LOOP
        INSERT INTO tb_tag (name, identifier)
        VALUES (tag_name, lower(tag_name))
        ON CONFLICT (identifier) DO UPDATE SET name = tag_name
        RETURNING id INTO tag_id;

        -- Link to post
        INSERT INTO tb_post_tag (fk_post, fk_tag)
        VALUES (
            (SELECT pk_post FROM tb_post WHERE id = new_post_id),
            (SELECT pk_tag FROM tb_tag WHERE id = tag_id)
        );

        created_tags := created_tags || jsonb_build_object('id', tag_id, 'name', tag_name);
    END LOOP;
...
```sql
-- Good: Return ID, let FraiseQL fetch full object
RETURNING id INTO new_id;
RETURN new_id;

-- Only return JSONB when you need computed fields
RETURN jsonb_build_object(
    'id', new_id,
    'computed_field', calculated_value  -- Not in view
);
```
...
```
Mutation executed successfully, but failed to fetch result.
ID {uuid} not found in v_user
```

**Cause:**
- Function returned an ID that doesn't exist in the table
- Projection table wasn't synced
...
```
Mutation result doesn't match expected type.
Expected field 'email' (String) but got null
```

**Cause:**
- JSONB structure doesn't match GraphQL type
...
### Browser-Based Schema Validator (Future)

A planned feature: **interactive schema validator** directly in your browser.

**Proposed workflow:**
1. Paste your schema definition (YAML/JSON/Python)
2. Validator checks mutation return shapes
3. Real-time error highlighting with line numbers
4. Suggestions for common fixes
...
│─────────────────────────────────────                                                                          │
│ ✅ Mutation: createUser             │                                                                          │
│    Returns: User (ID → v_user)                        │                                                       │
│                                                       │                                                       │
│ ⚠️  Mutation: transferFunds                           │                                                       │
│    Returns: JSONB                                     │                                                       │
│    Missing field: 'from_balance'                      │                                                       │
│    Hint: Add 'from_balance' field                     │                                                       │
│                                                       ↓                                                       │
│ ❌ Query: searchProducts            │                                                                          │
│    Returns: NonExistentType                           │                                                       │
↓    Suggestion: Did you mean...                        ↓                                                       ↓
└───────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
```

**This would validate:**
- Return type exists as a defined GraphQL type
- For ID returns: corresponding view (v_*) exists
- For JSONB returns: structure matches type definition
- Projection sync calls in function body
- Required fields in all types
