...
<D2Diagram name="observer-01-flow" alt="Observer flow: mutation triggers condition evaluation, queues actions for async execution" caption="Observer execution pipeline" />

Each observer can dispatch one or more action types:

<D2Diagram name="observer-02-actions" alt="Four action types: Webhook, Email, Slack, and Custom handlers" caption="Available action types" size="wide" />
